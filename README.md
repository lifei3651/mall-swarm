...
> 当前工作区已在原开源基座上实现“灵启商城”私有化版本。实际会员、奖金、商城与运维口径请以 [mall-distribution/document/README.md](mall-distribution/document/README.md) 和 [回归测试计划](mall-distribution/document/test-plan.md) 为准；下方内容是上游项目介绍，仅供技术背景参考。

## 当前有效工程边界

当前生产代码只包含以下有效工程：

- `mall-distribution`：当前核心业务后端
- `mall-mbg`：当前后端依赖的数据库访问层
- `mall-common`：当前后端依赖的公共层
- `mall-shop-web`：当前商城前端
- `mall-distribution-admin`：当前管理后台前端

旧版 `mall-admin`、`mall-portal`、`mall-search`、`mall-auth`、`mall-gateway`、`mall-monitor` 和 `mall-demo` 已从有效工程及根构建清单中隔离。详细边界、备份和恢复信息见 [旧模块隔离记录](document/legacy-module-isolation-20260722.md)。下方仍保留的旧版介绍只用于识别项目来源，不表示对应服务仍在运行或可以部署。
