...
describe('私域直播运营链路', () => {
  it('提供独立菜单、路由和真实接口', () => {
    ...
    const productMenu = layout.slice(layout.indexOf("key: 'products'"), layout.indexOf("key: 'orders'"))
    const shopSettingsMenu = layout.slice(layout.indexOf("key: 'operations'"), layout.indexOf("key: 'risk'"))
    expect(productMenu).not.toContain("title: '直播运营中心'")
    expect(shopSettingsMenu).toContain("title: '直播运营中心'")
    expect(shopSettingsMenu.indexOf("title: '商城视觉与页面'")).toBeLessThan(shopSettingsMenu.indexOf("title: '直播运营中心'"))
