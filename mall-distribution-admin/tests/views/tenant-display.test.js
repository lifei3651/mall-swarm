...
describe('商城视觉与页面工作台', () => {
  ...
  it('品牌文化改用多图详情并保留旧文字作为不可编辑兜底', async () => {
    ...
    expect(source).toContain('真正的旧文字仍可兜底，图片文件名或图片地址不会作为正文显示')
    expect(source).toContain('brandCultureContentWarning')
    expect(source).toContain('页面已开启，但还没有详情图')
    expect(source).toContain('目前仍在展示旧文字介绍')
    expect(source).toContain('safePreviewBrandCultureContent')
