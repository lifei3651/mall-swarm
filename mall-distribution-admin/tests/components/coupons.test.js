...
const api=vi.hoisted(()=>({listCoupons:vi.fn(),couponProducts:vi.fn(),couponMerchants:vi.fn(),saveCoupon:vi.fn(),changeCouponStatus:vi.fn(),getTenantBusinessModes:vi.fn(),store:{userInfo:{merchantId:null},hasPermission:()=>true}}))
vi.mock('@/api/coupons',()=>api)
vi.mock('@/api/tenant',()=>({getTenantBusinessModes:api.getTenantBusinessModes}))
...
beforeEach(()=>{vi.restoreAllMocks();vi.clearAllMocks();globalThis.ResizeObserver=class {observe(){} unobserve(){} disconnect(){}};api.store.userInfo.merchantId=null;api.listCoupons.mockResolvedValue({data:{list:[row()],total:1}});api.couponProducts.mockResolvedValue({data:[]});api.couponMerchants.mockResolvedValue({data:{list:[],totalPage:1}});api.getTenantBusinessModes.mockResolvedValue({data:{couponEnabled:1}});api.saveCoupon.mockResolvedValue({data:{}})})
describe('优惠券配置影响确认',()=>{
  ...
  it('模块关闭后只可读历史券并允许暂停，不能新建或发行',async()=>{
    api.getTenantBusinessModes.mockResolvedValue({data:{couponEnabled:0}})
    const w=mounted();await flushPromises()
    expect(w.vm.couponEnabled).toBe(false)
    w.vm.edit(row());expect(w.vm.readonly).toBe(true)
    expect(api.saveCoupon).not.toHaveBeenCalled()
    w.unmount()
  })
})
