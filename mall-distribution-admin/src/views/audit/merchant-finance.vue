...
const submitPay = async () => {
  await runAction(`pay-${current.value.id}`, async () => {
    await ElMessageBox.confirm(
      `确认银行已经实际打款 ¥${money(payForm.value.actualPaidAmount)}${payForm.value.paymentReference ? `，流水号 ${payForm.value.paymentReference.trim()}` : ''}？确认后将登记为已付款。`,
      '确认实际打款',
