<template>
  <div class="page-container merchant-finance-page">
    ...
    <el-tabs v-model="tab" @tab-change="loadCurrent">
      ...
      <el-tab-pane label="提现、发票与打款" name="withdrawals">
        <el-table :data="withdrawals" v-loading="loading" stripe>
          ...
          <el-table-column label="操作" width="390"><template #default="{ row }"><el-button link :disabled="Boolean(actionBusyKey)" @click="showEvents(row)">审批轨迹</el-button><el-button v-if="merchantCanManageFunds && ['SUBMITTED','INVOICE_PENDING'].includes(row.status)" link type="warning" :loading="actionBusyKey === `cancel-${row.id}`" :disabled="Boolean(actionBusyKey)" @click="cancelWithdrawal(row)">撤回</el-button><template v-if="canManage"><el-button v-if="['SUBMITTED','INVOICE_PENDING','READY_TO_PAY'].includes(row.status)" link type="primary" :disabled="Boolean(actionBusyKey)" @click="openReview(row)">发票/审核</el-button><el-button v-if="['READY_TO_PAY','PAYMENT_FAILED'].includes(row.status)" link type="success" :loading="actionBusyKey === `start-${row.id}`" :disabled="Boolean(actionBusyKey)" @click="startPayment(row)">开始付款</el-button><el-button v-if="row.status === 'PAYMENT_PROCESSING'" link type="success" :disabled="Boolean(actionBusyKey)" @click="openPay(row)">确认到账</el-button><el-button v-if="row.status === 'PAYMENT_PROCESSING'" link type="danger" :loading="actionBusyKey === `failed-${row.id}`" :disabled="Boolean(actionBusyKey)" @click="paymentFailed(row)">付款失败</el-button><el-button v-if="['SUBMITTED','INVOICE_PENDING','READY_TO_PAY','PAYMENT_PROCESSING','PAYMENT_FAILED'].includes(row.status)" link type="warning" :loading="actionBusyKey === `freeze-${row.id}`" :disabled="Boolean(actionBusyKey)" @click="riskFreeze(row)">风控冻结</el-button><el-button v-if="row.status === 'RISK_FROZEN'" link type="warning" :loading="actionBusyKey === `resume-${row.id}`" :disabled="Boolean(actionBusyKey)" @click="riskResume(row)">解除冻结</el-button><el-button v-if="['SUBMITTED','INVOICE_PENDING','READY_TO_PAY','PAYMENT_FAILED'].includes(row.status)" link type="danger" :loading="actionBusyKey === `reject-${row.id}`" :disabled="Boolean(actionBusyKey)" @click="reject(row)">驳回</el-button><el-button v-if="row.status === 'PAID'" link type="primary" :loading="actionBusyKey === `complete-${row.id}`" :disabled="Boolean(actionBusyKey)" @click="completeWithdrawalRecord(row)">归档完成</el-button></template></template></el-table-column>
        </el-table>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog v-model="reviewVisible" title="登记发票与打款调整" width="600px"><el-form label-width="120px"><el-form-item label="应开票金额"><el-input-number v-model="reviewForm.invoiceRequiredAmount" :min="0" :precision="2" style="width:100%" /></el-form-item><el-form-item label="已收票金额"><el-input-number v-model="reviewForm.invoiceReceivedAmount" :min="0" :precision="2" style="width:100%" /></el-form-item><el-form-item label="发票状态"><el-select v-model="reviewForm.invoiceStatus" style="width:100%"><el-option label="无需发票" value="NOT_REQUIRED"/><el-option label="待收发票" value="PENDING"/><el-option label="已收发票" value="RECEIVED"/></el-select></el-form-item><el-form-item label="调整金额"><el-input-number v-model="reviewForm.adjustmentAmount" :max="0" :precision="2" style="width:100%"/><div class="help">少打100元填写 -100；不自动认定为税费。</div></el-form-item><el-form-item label="调整原因"><el-input v-model="reviewForm.adjustmentReason" type="textarea" /></el-form-item></el-form><template #footer><el-button :disabled="Boolean(actionBusyKey)" @click="reviewVisible=false">取消</el-button><el-button type="primary" :loading="actionBusyKey === `review-${current?.id}`" @click="submitReview">保存审核</el-button></template></el-dialog>
    <el-dialog v-model="payVisible" title="确认实际打款" width="520px" @closed="payForm.adminPassword = ''"><el-form label-width="110px"><el-form-item label="实际打款"><el-input-number v-model="payForm.actualPaidAmount" :min="0.01" :precision="2" style="width:100%"/></el-form-item><el-form-item label="银行流水号"><el-input v-model="payForm.paymentReference" /></el-form-item><el-form-item label="凭证地址"><el-input v-model="payForm.paymentVoucherUrl" /></el-form-item><el-form-item label="管理员密码" required><el-input v-model="payForm.adminPassword" type="password" show-password maxlength="64" autocomplete="current-password" placeholder="二次验证当前管理员登录密码" /></el-form-item></el-form><template #footer><el-button :disabled="Boolean(actionBusyKey)" @click="payVisible=false">取消</el-button><el-button type="primary" :loading="actionBusyKey === `pay-${current?.id}`" @click="submitPay">确认已打款</el-button></template></el-dialog>
...
const submitApply = async () => { if (saving.value) return; saving.value = true; try { await applyMerchantWithdrawal(applyForm.value); ElMessage.success('提现申请已提交'); applyVisible.value = false; tab.value = 'withdrawals'; await loadCurrent() } finally { saving.value = false } }
...
const submitReview = async () => runAction(`review-${current.value.id}`, async () => { await ElMessageBox.confirm('确认保存本笔提现的发票状态和打款调整？', '确认保存财务审核', { type: 'warning' }); await reviewMerchantWithdrawal(current.value.id, reviewForm.value); ElMessage.success('发票与审核信息已保存'); reviewVisible.value = false; await loadCurrent() })
...
const submitPay = async () => {
  ...
  await runAction(`pay-${current.value.id}`, async () => {
    try {
...
const runAction = async (key, action) => { if (actionBusyKey.value) return; actionBusyKey.value = key; try { await action() } finally { actionBusyKey.value = '' } }
const reject = async (row) => runAction(`reject-${row.id}`, async () => { const { value } = await ElMessageBox.prompt('请填写驳回原因', '驳回商户提现', { inputValidator: (v) => Boolean(v?.trim()) || '必须填写原因' }); await rejectMerchantWithdrawal(row.id, { reason: value }); ElMessage.success('已驳回；冻结金额先抵退款欠款，剩余退回可提现余额'); await loadCurrent() })
const startPayment = async (row) => runAction(`start-${row.id}`, async () => { await ElMessageBox.confirm('确认已经开始向快照中的收款账户执行付款？进入付款处理中后仍需登记成功或失败结果。', '开始付款', { type: 'warning' }); await startMerchantWithdrawalPayment(row.id); ElMessage.success('已进入付款处理中'); await loadCurrent() })
const paymentFailed = async (row) => runAction(`failed-${row.id}`, async () => { const { value } = await ElMessageBox.prompt('请填写银行退回、账户异常等具体原因', '登记付款失败', { inputValidator: (v) => Boolean(v?.trim()) || '必须填写原因' }); await failMerchantWithdrawalPayment(row.id, { reason: value }); ElMessage.warning('已登记付款失败，申请金额仍保持冻结，可重试或驳回'); await loadCurrent() })
const cancelWithdrawal = async (row) => runAction(`cancel-${row.id}`, async () => { const { value } = await ElMessageBox.prompt('请填写撤回原因', '撤回提现申请', { inputValidator: (v) => Boolean(v?.trim()) || '必须填写原因' }); await cancelMerchantWithdrawal(row.id, { reason: value }); ElMessage.success('申请已撤回，冻结金额已按退款欠款规则退回'); await loadCurrent() })
const riskFreeze = async (row) => runAction(`freeze-${row.id}`, async () => { const { value } = await ElMessageBox.prompt('请填写风控冻结原因', '冻结提现流程', { inputValidator: (v) => Boolean(v?.trim()) || '必须填写原因' }); await freezeMerchantWithdrawal(row.id, { reason: value }); ElMessage.warning('提现流程已冻结，资金保持冻结不变'); await loadCurrent() })
const riskResume = async (row) => runAction(`resume-${row.id}`, async () => { const { value } = await ElMessageBox.prompt('请填写解除冻结原因', '恢复提现流程', { inputValidator: (v) => Boolean(v?.trim()) || '必须填写原因' }); await resumeMerchantWithdrawal(row.id, { reason: value }); ElMessage.success('已恢复到冻结前状态'); await loadCurrent() })
const completeWithdrawalRecord = async (row) => runAction(`complete-${row.id}`, async () => { await ElMessageBox.confirm('确认银行凭证、发票和审批资料均已归档？', '归档完成'); await completeMerchantWithdrawal(row.id); ElMessage.success('提现单已完成归档'); await loadCurrent() })
