...
const integrations=ref([]), tasks=ref([]), loading=ref(false), visible=ref(false), saving=ref(false), retryingId=ref(null)
...
async function save(){
  ...
  if (form.value.callbackToken && form.value.callbackToken.length < 32) return ElMessage.warning('ERP回调令牌至少需要32位')
  await ElMessageBox.confirm(form.value.enabled ? '保存后将启用ERP自动推单，请确认接口环境、地址和密钥均已核对。' : '确认保存当前ERP配置？', '确认保存ERP配置', { type: form.value.enabled ? 'warning' : 'info' })
  saving.value=true
  try {
    const payload={...form.value,appSecret:form.value.appSecret?.trim()||null,callbackToken:form.value.callbackToken?.trim()||null}
    await saveErpIntegration(payload); ElMessage.success('ERP配置已保存'); visible.value=false; await load()
  } finally { saving.value=false }
}
async function retry(row){
  if (retryingId.value !== null) return
  await ElMessageBox.confirm(`确认重新向 ${providerName(row.providerCode)} 推送任务 ${row.taskNo}？请先确认ERP侧未处理成功，避免重复业务。`, '确认重试ERP推送', { type:'warning' })
  retryingId.value=row.id
  try { const ok=(await retryErpTask(row.id)).data; ElMessage[ok?'success':'warning'](ok?'推送成功':'推送未成功，请检查配置'); await load() } finally { retryingId.value=null }
}
onMounted(load)
</script>
