...
const registrationNotice = '\n\n14. 首次手机号注册后的可选资料填写：只有你主动选择头像、填写昵称并点击“保存并进入商城”后才向商城提交这些资料；点击“暂不设置”不提交。所选图片和未保存昵称仅临时保留，退出填写时清理。该步骤不自动读取微信头像或昵称，跳过不影响账号使用。账号资料页更换头像仍在主动选择完成后提交。'
module.exports = { titles, resolveText, content, faqs, miniPrivacy: miniPrivacy + miniClientNotice.replace('换绑手机号、注销账号及其他尚无自助入口的请求需联系客服', '换绑手机号可在账号资料内自助办理；旧手机号已停用、注销账号及其他尚无自助入口的请求需联系客服') + miniInviteNotice + accountNotice + liveNotice + nativeAccountNotice + registrationNotice, contactValue }
