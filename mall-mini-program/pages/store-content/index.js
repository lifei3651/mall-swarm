...
Page({
  ...
  async load() {
    this.setData({ loading: true, error: '', videoFailed: false })
    try {
      ...
      this.setData({ ...theme.remember(home), type: this.contentType })
      ...
      if (this.contentType === 'culture') {
        ...
        this.setData({ room, videoUrl, products: (room.products || []).map(format.product) })
        ...
        this.setData({ rooms: (rooms || []).filter((item) => item.room).map((item) => ({ ...item, room: { ...item.room, coverUrl: format.mediaUrl(item.room.coverUrl) } })) })
      }
    } catch (error) { this.setData({ error: error.message || '内容加载失败' }) }
    finally { this.setData({ loading: false }) }
  },
  productImageError(event) {
    const index = Number(event.currentTarget.dataset.index)
    if (Number.isInteger(index) && this.data.products[index]) this.setData({ [`products[${index}].imageFailed`]: true })
  },
  detailImageError(event) {
    const index = Number(event.currentTarget.dataset.index)
    if (Number.isInteger(index) && this.data.culture.detailImages[index]) this.setData({ [`culture.detailImages[${index}].failed`]: true })
  },
  videoError() { this.setData({ videoFailed: true }) },
