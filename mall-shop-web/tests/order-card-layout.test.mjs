...
test('H5订单操作不换行、主按钮固定最右，取消和进行状态使用统一色阶', async () => {
  ...
  assert.match(view, /'退换\/售后'/)
  assert.match(view, /'取消并退款'/)
...
test('H5订单详情按状态、本人收入、物流收货人、商品和全部信息顺序收口', async () => {
  ...
  assert.match(view, /addressExpanded \? \(fullRecipient \|\| '收货人信息未填写'\)/)
  assert.match(view, /<small :class="\{ 'is-expanded': addressExpanded \}">\{\{ addressExpanded \? order\.receiverAddress/)
  assert.match(view, /\.consumer-recipient-row \.is-expanded \{[^}]*white-space: normal;/)
