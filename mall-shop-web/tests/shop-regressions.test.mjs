...
test('checkout limits address fields, avoids persisting recipient PII and clears sensitive payment state', async () => {
  ...
  assert.match(checkout, /v-model="remarkDraft"[^>]*maxlength="500"/)
  assert.match(checkout, /form.value.remark = remarkDraft.value.slice\(0, 500\).trim\(\)/)
...
test('checkout keeps the address block compact and preserves the remark while switching', async () => {
  ...
  assert.match(source, /v-model="remarkDraft"/)
...
test('wallet transfer is packaged only by integrated H5 and requires verified adult accounts', async () => {
  ...
  assert.match(team, /开通推广资格后可查询/)
  assert.match(team, /当前账号尚未开通推广资格/)
...
test('fulfilled orders show auto-receipt protection, logistics after-sale reasons and per-package actions', async () => {
  ...
  assert.match(detail, /startExceptionRefund/)
  assert.match(detail, /applyExceptionRefund/)
...
test('pending reviews lock the exact order item and after-sale disputes show deadlines and reapply entry', async () => {
  ...
  assert.doesNotMatch(productDetail, /写评价|submitProductReview|reviewFormVisible/)
  const reviewPage = await readView('OrderReviewView.vue')
  assert.match(reviewPage, /orderItemId: reviewOrderItemId\.value/)
  assert.match(orders, /\/order-review\//)
  assert.match(orderDetail, /\/order-review\//)
