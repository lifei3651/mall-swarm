<template>
  <main class="support-page" :aria-busy="loading">
    ...
    <nav class="status-tabs" aria-label="工单状态">
      <button v-for="item in filters" :key="item.key" :class="{active:status===item.key}" :aria-pressed="status===item.key" @click="changeStatus(item.key)">{{ item.label }}</button>
    </nav>
