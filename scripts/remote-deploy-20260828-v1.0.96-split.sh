...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.96
EXPECTED_VERSION=1.0.96
EXPECTED_PREVIOUS_VERSION=1.0.95
EXPECTED_PREVIOUS_GIT_COMMIT=c82a5e1dce360d7c38ef0ca470b5a1ee76496a9d
EXPECTED_PREVIOUS_BUILD_ID=20260828-1240-1.0.95
EXPECTED_GIT_COMMIT=7060ab820373534847dde3cd1426c6412452985e
EXPECTED_BUILD_ID=20260828-1340-1.0.96
EXPECTED_JAR_SHA256=9dbdb2c46897e0420f086d59fa2244b619283615f3e3fdf82cd8e98550c3e785
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1096
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1096\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.96 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.96"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.96.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.96.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.96.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.96.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.96"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.96 startup log verification failed" >&2
  exit 1
fi
