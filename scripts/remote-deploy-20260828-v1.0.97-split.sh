...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.97
EXPECTED_VERSION=1.0.97
EXPECTED_PREVIOUS_VERSION=1.0.96
EXPECTED_PREVIOUS_GIT_COMMIT=7060ab820373534847dde3cd1426c6412452985e
EXPECTED_PREVIOUS_BUILD_ID=20260828-1340-1.0.96
EXPECTED_GIT_COMMIT=a95ac56272e797c0ae3ab90a669a548ccbffe7e6
EXPECTED_BUILD_ID=20260828-1428-1.0.97
EXPECTED_JAR_SHA256=69680ae061b1998db4ac0faec7da498c5ea1c324649c035e6fe629985c274ad4
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1097
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1097\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.97 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.97"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.97.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.97.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.97.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.97.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.97"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.97 startup log verification failed" >&2
  exit 1
fi
