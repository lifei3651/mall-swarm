#!/usr/bin/env bash
# admin static only; preserve other sites, backend, configuration and database.
set -Eeuo pipefail
umask 077
...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-admin-137\.[A-Za-z0-9]+$ ]]
...
with tarfile.open(sys.argv[1] + '/files-and-config.tar.gz') as archive:
    for filename in ['index.html', 'version.json']:
        assert archive.extractfile('opt/lingqimall/nginx/admin/' + filename).read() == pathlib.Path(sys.argv[2], filename).read_bytes()
PY
  printf '%s\n' "$backup_path"
}
recover() {
  ...
  if [[ "$status" != 0 && "$MUTATED" == 1 && -d "$ROLLBACK/admin" ]]; then
    ...
    echo "admin-recovery=previous-static-restored backup=$ROLLBACK"
  fi
  exit "$status"
}
trap recover EXIT
...
assert version == {'version':'1.0.137','edition':'app-h5-split','application':'admin','gitCommit':commit,'buildId':'20260909-admin-1.0.137'}
print('admin-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'admin-preflight=passed previous=1.0.136 target=1.0.137 backend=1.0.137'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/admin-137.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.admin-137-stage.XXXXXX")
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "admin-release-success version=1.0.137 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
