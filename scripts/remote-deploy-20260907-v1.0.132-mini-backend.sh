...
umask 077
EXPECTED_VERSION=1.0.132
EXPECTED_PREVIOUS_VERSION=1.0.131
OLD_JAR_SHA=98e23533774e116b3ade453fdeba13d20dff75c6b8e927c77af6bbc37bbd6995
APP_ROOT=/opt/lingqimall
DB_NAME=mall_distribution
SERVICE=lingqimall-distribution.service
RELEASE_DIR=${1:-}
MODE=${2:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-mini-backend-132\.[A-Za-z0-9]+$ ]]
...
with open(root + '/RELEASE_MANIFEST.json') as f:
    m = json.load(f)
assert m['version'] == '1.0.132' and m['scope'] == 'backend-only'
assert m['gitCommit'] == '91e5a6d824be2f6e7a3f6f9f5d642b7f9635c957'
assert m['buildId'] == '20260907-mini-backend-1.0.132'
...
with zipfile.ZipFile(root + '/mall-distribution.jar') as z:
    assert 'BOOT-INF/classes/com/macro/mall/distribution/service/WeChatMiniProgramBonusService.class' in z.namelist()
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/mini-backend-132.XXXXXX)
...
VERIFY_DB=mall_distribution_release_verify_132_$(date +%Y%m%d%H%M%S)
[[ "$VERIFY_DB" =~ ^mall_distribution_release_verify_132_[0-9]{14}$ ]]
