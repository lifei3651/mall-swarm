...
const BUILD_ID = '20260921-closure-1.0.153'
const PREVIOUS_BACKEND_VERSION = '1.0.152'
const PREVIOUS_BACKEND_JAR_SHA256 = '5acbcd7b43c2b41a6aa6db685fbdbffa0fb39d17a01ae694fa3d5c29b4273893'
const PREVIOUS_STATIC_VERSION = '1.0.151'
const PREVIOUS_STATIC_COMMIT = 'e25c760c4428fcc65956847adbf8bbb252f76db3'

if (process.argv.length !== 3 || process.argv[2] !== '--package-only') {
  throw new Error('Usage: node scripts/release-lingqi-153.mjs --package-only')
}
...
if (version !== VERSION || miniPackage.version !== VERSION || miniLock.version !== VERSION
  ...
  throw new Error('Root and mini-program versions must all equal 1.0.153')
}
...
const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-153-unified-candidate.'))
...
copy('scripts/remote-deploy-20260921-v1.0.153-backend.sh', path.join(stage, 'release-backend.sh'), 0o755)
copy('scripts/remote-deploy-20260921-v1.0.153-static.sh', path.join(stage, 'release-static.sh'), 0o755)
...
const miniStage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-153-mini-source.'))
