...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.91
EXPECTED_VERSION=1.0.91
EXPECTED_PREVIOUS_VERSION=1.0.90
EXPECTED_PREVIOUS_GIT_COMMIT=e0c3da131e3d8d762e8cc36e33d36389f5826bdc
EXPECTED_PREVIOUS_BUILD_ID=20260825-1747-1.0.90
EXPECTED_GIT_COMMIT=6f7862a33b01adf4f3233874591d7f2b92fa5fd7
EXPECTED_BUILD_ID=20260826-1502-1.0.91
EXPECTED_JAR_SHA256=89c38903fa8c4c50842a23cd0d02652357b78bcdc7b666349790ac2c9038949b
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.91 release failed; entering bounded recovery" >&2
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.91.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.91.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.91.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.91"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.91 startup log verification failed" >&2
  exit 1
fi
