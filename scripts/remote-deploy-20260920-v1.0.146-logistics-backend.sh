#!/usr/bin/env bash
# Backend prerequisite for the mini-program official logistics query component.
# Preserve database, static sites, secrets and runtime configuration.
set -Eeuo pipefail
shopt -s inherit_errexit
umask 077

EXPECTED_VERSION=1.0.146
EXPECTED_PREVIOUS_VERSION=1.0.145
OLD_JAR_SHA=e2df05770cac70d97436298f8631e3e4e6803d542bc38323fbf8403e5d54fd55
NEW_JAR_SHA=014df28fa9123b1c00d51e2a4aa075dd5ea8b56e95ac4ce7f4528c6ca844015e
APP_ROOT=/opt/lingqimall
DB_NAME=mall_distribution
SERVICE=lingqimall-distribution.service
RELEASE_DIR=${1:-}
MODE=${2:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-logistics-backend-146\.[A-Za-z0-9]+$ ]]
...
assert manifest['version'] == '1.0.146' and manifest['scope'] == 'mini-logistics-backend-only'
assert manifest['productionBaselineJarSha256'] == 'e2df05770cac70d97436298f8631e3e4e6803d542bc38323fbf8403e5d54fd55'
...
with zipfile.ZipFile(root + '/mall-distribution.jar') as archive:
    ...
    assert 'BOOT-INF/classes/com/macro/mall/distribution/service/WeChatLogisticsQueryService.class' in names
    assert 'BOOT-INF/classes/com/macro/mall/distribution/vo/WeChatWaybillTokenVO.class' in names
    assert 'BOOT-INF/classes/com/macro/mall/distribution/wechat/OfficialWeChatMiniProgramGateway.class' in names
...
[[ "$(curl -sS -o /dev/null -w '%{http_code}' --max-time 12 -X POST 'https://lingqimall.com/api/shop/orders/1/wechat-logistics-token?shipmentId=1' -H 'X-Shop-Client: wechat-mini-program' -H 'X-Shop-Surface: mini-program')" == 404 ]]
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/logistics-backend-146.XXXXXX)
...
[[ "$(curl -sS -o /dev/null -w '%{http_code}' --max-time 12 -X POST 'https://lingqimall.com/api/shop/orders/1/wechat-logistics-token?shipmentId=1' -H 'X-Shop-Client: wechat-mini-program' -H 'X-Shop-Surface: mini-program')" == 401 ]]
sleep 8
...
MUTATED=0
echo "release-success version=$EXPECTED_VERSION backup-before=$BACKUP_BEFORE backup-after=$BACKUP_AFTER rollback=$ROLLBACK_DIR migrations=36:36 logistics-token-auth=401 database-static-config-preserved=yes"
