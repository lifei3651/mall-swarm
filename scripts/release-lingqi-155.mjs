...
const BUILD_ID = '20260922-closure-1.0.155'
...
if (process.argv.length !== 3 || process.argv[2] !== '--package-only') {
  throw new Error('Usage: node scripts/release-lingqi-155.mjs --package-only')
}
...
if (version !== VERSION || miniPackage.version !== VERSION || miniLock.version !== VERSION
  ...
  throw new Error('Root and mini-program versions must all equal 1.0.155')
}
...
const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-155-unified-candidate.'))
...
copy('scripts/remote-deploy-20260922-v1.0.155-backend.sh', path.join(stage, 'release-backend.sh'), 0o755)
copy('scripts/remote-deploy-20260922-v1.0.155-static.sh', path.join(stage, 'release-static.sh'), 0o755)
...
const miniStage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-155-mini-source.'))
