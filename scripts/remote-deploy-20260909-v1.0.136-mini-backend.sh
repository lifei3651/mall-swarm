...
umask 077
EXPECTED_VERSION=1.0.136
EXPECTED_PREVIOUS_VERSION=1.0.134
OLD_JAR_SHA=24a5b857e8992ba8ea845b808757658e94168bc7e3d69eb34e7ff59780c17db9
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-mini-backend-136\.[A-Za-z0-9]+$ ]]
...
with open(root + '/RELEASE_MANIFEST.json') as f:
    m = json.load(f)
assert m['version'] == '1.0.136' and m['scope'] == 'backend-only'
...
assert m['buildId'] == '20260909-mini-backend-1.0.136'
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/mini-backend-136.XXXXXX)
...
VERIFY_DB=mall_distribution_release_verify_136_$(date +%Y%m%d%H%M%S)
[[ "$VERIFY_DB" =~ ^mall_distribution_release_verify_136_[0-9]{14}$ ]]
