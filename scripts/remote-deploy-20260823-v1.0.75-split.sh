...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.75
EXPECTED_VERSION=1.0.75
EXPECTED_PREVIOUS_VERSION=1.0.74
EXPECTED_GIT_COMMIT=a56c12cb9f83f66c9c68ca293a0569b8ad371a54
EXPECTED_BUILD_ID=20260823-0910-1.0.75
EXPECTED_JAR_SHA256=d9b88e681174c5b0b533489dac420a1f8151a451f0a8e8914d3ef5f6403e1589
EXPECTED_PREVIOUS_MIGRATIONS=18
...
rollback() {
  local code=$?
  if [[ "$code" != 0 ]]; then
    echo "1.0.75 release failed; entering bounded recovery" >&2
    ...
    if [[ "$APP_MUTATED" == 1 && "$ENCRYPTION_STARTED" == 0 ]]; then
      ...
      rm -f -- "$APP_ROOT/config/data-encryption.env" /etc/systemd/system/lingqimall-distribution.service.d/data-encryption.conf
      systemctl daemon-reload || true
      systemctl start lingqimall-distribution.service || true
    elif [[ "$ENCRYPTION_STARTED" == 1 ]]; then
      echo "sensitive-data encryption has started; retaining the 1.0.75 compatible backend instead of restoring 1.0.74" >&2
...
[[ "$RELEASE_DIR" == /tmp/lingqimall-release-v1.0.75 ]]
for file in mall-distribution.jar admin.tar.gz shop.tar.gz team.tar.gz VERSION SHA256SUMS production-backup.sh db-migrate.sh lingqimall.conf lingqimall-security.conf; do
  [[ -s "$RELEASE_DIR/$file" ]]
done
...
BEFORE_SENSITIVE_VALUES="$(sensitive_value_count)"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.75.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.75.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.75.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.75.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  ...
  grep -q '"version": "1.0.75"' "$manifest"
...
install -d -m 0750 -o root -g lingqimall /etc/systemd/system/lingqimall-distribution.service.d
umask 0027
data_key="$(openssl rand -hex 32)"
[[ "$data_key" =~ ^[0-9a-f]{64}$ ]]
printf 'DATA_ENCRYPTION_KEY=%s\nDATA_ENCRYPTION_WRITE_ENABLED=false\nDATA_ENCRYPTION_MIGRATE_ON_STARTUP=true\n' "$data_key" > "$APP_ROOT/config/data-encryption.env"
unset data_key
chown root:lingqimall "$APP_ROOT/config/data-encryption.env"
chmod 0640 "$APP_ROOT/config/data-encryption.env"
printf '[Service]\nEnvironmentFile=/opt/lingqimall/config/data-encryption.env\n' > /etc/systemd/system/lingqimall-distribution.service.d/data-encryption.conf
chmod 0644 /etc/systemd/system/lingqimall-distribution.service.d/data-encryption.conf
...
wait_for_health
[[ "$(database_counts)" == "$BEFORE_COUNTS" ]]
[[ "$(plaintext_sensitive_value_count)" == "$BEFORE_SENSITIVE_VALUES" ]]
curl -fsS --max-time 12 http://127.0.0.1:8086/shop/home >/dev/null

# 第二阶段：新程序已证明可兼容读取旧明文，此后只保留支持解密的 1.0.75 后端。
sed -i 's/^DATA_ENCRYPTION_WRITE_ENABLED=false$/DATA_ENCRYPTION_WRITE_ENABLED=true/' "$APP_ROOT/config/data-encryption.env"
grep -qx 'DATA_ENCRYPTION_WRITE_ENABLED=true' "$APP_ROOT/config/data-encryption.env"
ENCRYPTION_STARTED=1
systemctl restart lingqimall-distribution.service
wait_for_health
for _ in $(seq 1 30); do
  [[ "$(plaintext_sensitive_value_count)" == 0 ]] && break
  sleep 2
done
[[ "$(plaintext_sensitive_value_count)" == 0 ]]
[[ "$(sensitive_value_count)" == "$BEFORE_SENSITIVE_VALUES" ]]
[[ "$(database_counts)" == "$BEFORE_COUNTS" ]]
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.75 startup log verification failed" >&2
  exit 1
fi
...
echo "release-success version=$EXPECTED_VERSION backup-before=$BACKUP_BEFORE backup-after=$BACKUP_AFTER build=$EXPECTED_BUILD_ID core-counts=$BEFORE_COUNTS migrations=$EXPECTED_MIGRATIONS encrypted-values=$BEFORE_SENSITIVE_VALUES jar=$EXPECTED_JAR_SHA256"
