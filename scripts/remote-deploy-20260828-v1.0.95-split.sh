...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.95
EXPECTED_VERSION=1.0.95
EXPECTED_PREVIOUS_VERSION=1.0.94
EXPECTED_PREVIOUS_GIT_COMMIT=cc420456aa64a4ce173074904bd85a801cc6ae29
EXPECTED_PREVIOUS_BUILD_ID=20260828-1012-1.0.94
EXPECTED_GIT_COMMIT=c82a5e1dce360d7c38ef0ca470b5a1ee76496a9d
EXPECTED_BUILD_ID=20260828-1240-1.0.95
EXPECTED_JAR_SHA256=53903dbeca78b7df79d358e1678b8b79a29991901d44a08b3a2574bbdf5a2e41
EXPECTED_PREVIOUS_MIGRATIONS=27
...
verify_service_ticket_schema() {
  [[ "$(service_ticket_table_count)" == 2 ]]
  [[ "$(mysql_root -NBe "SELECT COUNT(*) FROM information_schema.columns
    WHERE table_schema='${DB_NAME}' AND table_name='dms_shop_service_ticket'
  ...
  [[ "$(mysql_root -NBe "SELECT COUNT(DISTINCT CONCAT(table_name,':',index_name)) FROM information_schema.statistics
    WHERE table_schema='${DB_NAME}' AND index_name IN
  ...
  [[ "$(mysql_root -NBe "SELECT COUNT(*) FROM information_schema.referential_constraints
    WHERE constraint_schema='${DB_NAME}' AND table_name IN ('dms_shop_service_ticket','dms_shop_service_ticket_reply')")" == 0 ]]
  [[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_shop_service_ticket')" == 0 ]]
  [[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_shop_service_ticket_reply')" == 0 ]]
}
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1095
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1095\`" >/dev/null 2>&1 || true' EXIT
    ...
    [[ "$(service_ticket_table_count_for "$verify_db")" == 0 ]]
  )
}
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.95 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.95"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.95.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.95.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.95.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.95.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.95"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.95 startup log verification failed" >&2
  exit 1
fi
