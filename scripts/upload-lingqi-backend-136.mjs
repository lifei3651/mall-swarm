...
if (fs.readFileSync('VERSION', 'utf8').trim() !== '1.0.136') throw new Error('Wrong version')
...
const remote = cp.execFileSync('ssh', [...options, host, 'mktemp -d /tmp/lingqimall-mini-backend-136.XXXXXX'], { encoding: 'utf8' }).trim()
if (!/^\/tmp\/lingqimall-mini-backend-136\.[A-Za-z0-9]+$/.test(remote)) throw new Error('Unexpected remote staging directory')
