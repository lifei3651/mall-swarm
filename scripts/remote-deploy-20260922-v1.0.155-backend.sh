#!/usr/bin/env bash
# 1.0.155 closure backend release. Exactly one additive service-tag snapshot migration is allowed.
set -Eeuo pipefail
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-155\.[A-Za-z0-9]+$ ]] || fail "invalid release directory"
...
verify_unauthorized_contract() {
  local body status
  body=$(mktemp /tmp/lingqimall-closure-155-unauthorized.XXXXXX)
...
atomic_install() {
  ...
  temporary=$(mktemp "$(dirname "$destination")/.release-155.$(basename "$destination").XXXXXX") || return 1
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/closure-backend-155.XXXXXX)
...
VERIFY_DB=m155v_$(date +%Y%m%d%H%M%S)
[[ "$VERIFY_DB" =~ ^m155v_[0-9]{14}$ ]] || fail "invalid verification database"
