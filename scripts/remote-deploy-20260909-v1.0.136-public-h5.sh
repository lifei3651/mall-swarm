...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-public-h5-136\.[A-Za-z0-9]+$ ]]
...
assert old['version'] == '1.0.135' and old['gitCommit'] == 'c6f72305f25d1a02b4e05e805a63c4df2072968e'
...
assert version == {'version':'1.0.136','edition':'app-h5-split','application':'storefront-public','gitCommit':commit,'buildId':'20260909-public-h5-1.0.136'}
print('public-h5-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'public-h5-preflight=passed previous=1.0.135 target=1.0.136 backend=1.0.136'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/public-h5-136.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.shop-136-stage.XXXXXX")
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "public-h5-release-success version=1.0.136 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
