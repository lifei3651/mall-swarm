...
usage() {
  cat <<'EOF'
用法：scripts/release-readiness-165.sh [选项]
...
expected_previous_backend_version = "1.0.158"
expected_previous_backend_jar = "cbd249426c1dfcdad4de7dbf65f9780486b47a9807e35ae3eb27fb8252f1ae1f"
expected_previous_static_version = "1.0.158"
expected_previous_static_commit = "fcadf4099861a0a5359b12761cbf232c160635a7"
...
if len(repo_migrations) != expected_migration_count or repo_migrations[-1] != expected_last_migration:
    fail("repository migration inventory is not the fixed 1.0.165 set")
...
if sha_file(candidate / "document/db/migrations" / expected_last_migration) != expected_last_migration_sha:
    fail("1.0.165 service-tag migration checksum mismatch")
...
PY
  ...
  for marker in 'business_snapshot' 'protected_hashes' 'verify_unauthorized_contract' \
    ...
    'migration-mode=verify-only'; do
    grep -Fq -- "$marker" "$CANDIDATE_ROOT/release-backend.sh" \
      || fail "候选后端发布脚本缺少收口保护：$marker"
  done
  ! grep -Eq 'db-migrate\.sh[" ]+apply|mysql_db "\$DB_NAME" <' "$CANDIDATE_ROOT/release-backend.sh" \
    || fail "1.0.165 禁止重跑正式库迁移 SQL"
