...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.110
EXPECTED_VERSION=1.0.110
EXPECTED_PREVIOUS_VERSION=1.0.109
EXPECTED_PREVIOUS_GIT_COMMIT=c3f3e014f01fb252fa15d4971382c19e591441ee
EXPECTED_PREVIOUS_BUILD_ID=20260901-0914-1.0.109
EXPECTED_GIT_COMMIT=c27e14b86e9b86cc05ddef58490dbe8e2eda9f9f
EXPECTED_BUILD_ID=20260901-1431-1.0.110
EXPECTED_JAR_SHA256=8b85c6a5d9082ad23a5842e7c95840e17acf6ce976cbcd675036120099901a8a
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1110
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1110\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.110 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.110"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.110.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.110.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.110.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.110.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.110"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.110 startup log verification failed" >&2
  exit 1
fi
