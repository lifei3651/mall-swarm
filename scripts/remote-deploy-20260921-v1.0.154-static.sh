#!/usr/bin/env bash
# Deploy exactly one immutable 1.0.154 static surface from the unified closure candidate.
set -Eeuo pipefail
...
EXPECTED_PREVIOUS_COMMIT=8d94be83088f3ed8aab8664a7a13eccdffc4cb41
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-154\.[A-Za-z0-9]+$ ]] || fail "invalid candidate directory"
...
case "$SITE" in
  admin)
    ...
    PREVIOUS_BUILD_ID=20260921-closure-1.0.153
    ;;
  shop)
    LABEL=public-h5
...
assert manifest['previousStaticVersion'] == '1.0.153'
assert manifest['previousStaticCommit'] == '8d94be83088f3ed8aab8664a7a13eccdffc4cb41'
...
assert candidate == {
    'version':'1.0.154', 'edition':'app-h5-split', 'application':application,
    'gitCommit':commit, 'buildId':build_id,
}
...
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "$EXPECTED_VERSION" ]] || fail "backend version is not 1.0.154"
...
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/$LABEL-154.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.$SITE-154-stage.XXXXXX")
...
query = '?release=1.0.154-' + commit[:12]
