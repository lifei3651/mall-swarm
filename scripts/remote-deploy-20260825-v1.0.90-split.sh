...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.90
EXPECTED_VERSION=1.0.90
EXPECTED_PREVIOUS_VERSION=1.0.89
EXPECTED_PREVIOUS_GIT_COMMIT=a203be3aa4e3d7284adf477a9549bde921a549a0
EXPECTED_PREVIOUS_BUILD_ID=20260825-1632-1.0.89
EXPECTED_GIT_COMMIT=e0c3da131e3d8d762e8cc36e33d36389f5826bdc
EXPECTED_BUILD_ID=20260825-1747-1.0.90
EXPECTED_JAR_SHA256=cfb410ab737c8307529c93facdcfa8b96af52399f10367544368b5939d258d6e
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1090
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1090\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.90 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.90"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.90.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.90.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.90.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.90.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.90"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.90 startup log verification failed" >&2
  exit 1
fi
