...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.94
EXPECTED_VERSION=1.0.94
EXPECTED_PREVIOUS_VERSION=1.0.93
EXPECTED_PREVIOUS_GIT_COMMIT=132f2defa59e8e6051f24fec0d23752498ef6833
EXPECTED_PREVIOUS_BUILD_ID=20260828-0835-1.0.93
EXPECTED_GIT_COMMIT=cc420456aa64a4ce173074904bd85a801cc6ae29
EXPECTED_BUILD_ID=20260828-1012-1.0.94
EXPECTED_JAR_SHA256=c60a2261b96e0854696e16ce87e3fd427e2b9bb6346059e978715f20bc45eef3
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1094
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1094\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.94 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.94"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.94.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.94.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.94.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.94.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.94"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.94 startup log verification failed" >&2
  exit 1
fi
