...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.80
EXPECTED_VERSION=1.0.80
EXPECTED_PREVIOUS_VERSION=1.0.76
EXPECTED_GIT_COMMIT=f2ce2142188b8a023c27c9412144423ac468c090
EXPECTED_BUILD_ID=20260823-1631-1.0.80
EXPECTED_JAR_SHA256=db26f590136da6aaa9fc1f2593090873d295760957927b7c37841cec57713b65
EXPECTED_PREVIOUS_MIGRATIONS=19
...
rollback() {
  local code=$?
  if [[ "$code" != 0 ]]; then
    echo "1.0.80 release failed; entering bounded recovery" >&2
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.80.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.80.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.80.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.80.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  ...
  grep -q '"version": "1.0.80"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.80 startup log verification failed" >&2
  exit 1
fi
