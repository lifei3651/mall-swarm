...
const buildId = '20260923-closure-1.0.158'
...
function temporaryRepository(files) {
  const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'closure-158-contract.'))
...
test('1.0.158 unified candidate is package-only and binds the 1.0.154 baseline', () => {
  const source = read('release-lingqi-158.mjs')
  assert.match(source, /const VERSION = '1\.0\.158'/)
  assert.match(source, /const BUILD_ID = '20260923-closure-1\.0\.158'/)
  ...
  assert.match(source, /remote-deploy-20260923-v1\.0\.158-backend\.sh/)
  assert.match(source, /remote-deploy-20260923-v1\.0\.158-static\.sh/)
...
test('1.0.158 WeChat wrapper can only preflight or upload a development version', () => {
  const source = read('upload-lingqi-mini-158.mjs')
  assert.match(source, /const VERSION = '1\.0\.158'/)
...
test('WeChat preflight executes only harmless CLI capability probes and never upload', () => {
  const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'closure-158-wechat-preflight.'))
  const root = path.join(parent, 'mall-swarm-app-h5')
  ...
  const transformed = read('upload-lingqi-mini-158.mjs')
  ...
  fs.writeFileSync(path.join(root, 'VERSION'), '1.0.158\n')
  fs.writeFileSync(path.join(root, 'scripts/upload-lingqi-mini-158.mjs'), transformed)
  ...
  fs.writeFileSync(path.join(project, 'package.json'), JSON.stringify({ version: '1.0.158' }))
  fs.writeFileSync(path.join(project, 'package-lock.json'), JSON.stringify({
    version: '1.0.158', packages: { '': { version: '1.0.158' } },
  }))
...
`)
  ...
  try {
    const result = cp.spawnSync(process.execPath, ['scripts/upload-lingqi-mini-158.mjs', '--preflight-only', '--port', '47777'], {
      cwd: root,
      encoding: 'utf8',
    })
...
test('1.0.158 backend allows only the fixed additive migration and never drops it on recovery', () => {
  const source = read('remote-deploy-20260923-v1.0.158-backend.sh')
  const migrationCountSql = 'SELECT CONCAT(COUNT(*),":",SUM(success=1)) FROM dms_schema_migration_history'
  assert.equal(source.split(migrationCountSql).length - 1, 2,
    'both migration-count probes must pass executable SQL without shell-escaped quotes')
  assert.doesNotMatch(source, /\\":\\"/,
    'a backslash inside the single-quoted mysql -e argument is sent literally and breaks MySQL')
  ...
  assert.match(source, /manifest\.get\('buildId'\) == build_id == '20260923-closure-1\.0\.158'/)
...
test('1.0.158 static release binds the exact build method and independently supplied source tree', () => {
  const source = read('remote-deploy-20260923-v1.0.158-static.sh')
  assert.match(source, /EXPECTED_BUILD_ID=20260923-closure-1\.0\.158/)
...
test('generic readiness and mini preparation point at the same 1.0.158 identity', () => {
  ...
  for (const source of [prepare, readiness]) {
    assert.match(source, /1\.0\.158/)
    assert.match(source, /20260923-closure-1\.0\.158/)
    assert.match(source, new RegExp(previousJar))
    assert.match(source, new RegExp(previousCommit))
  }
  ...
  assert.match(regression, /EXPECTED_VERSION=1\.0\.158/)
  assert.match(regression, /EXPECTED_BUILD_ID=20260923-closure-1\.0\.158/)
  ...
  assert.match(regression, /release-lingqi-158\.mjs/)
  assert.match(regression, /upload-lingqi-mini-158\.mjs/)
})
...
test('regression gate rejects a wrong build id before running any build command', () => {
  ...
  try {
    ...
    assert.match(result.stderr, /RELEASE_BUILD_ID 必须精确等于 20260923-closure-1\.0\.158/)
