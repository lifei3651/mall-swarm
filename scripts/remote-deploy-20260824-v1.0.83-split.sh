...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.83
EXPECTED_VERSION=1.0.83
...
EXPECTED_GIT_COMMIT=41b0073a9318c49d875b727fe61bf31b0df287c9
EXPECTED_BUILD_ID=20260824-1540-1.0.83
EXPECTED_JAR_SHA256=c3eda77ebd9dc4072456d0e3f6c2f15c1c06fac369860c3d635f5b87dc697320
EXPECTED_PREVIOUS_MIGRATIONS=22
EXPECTED_MIGRATIONS=23
EXPECTED_NEW_MIGRATION=202608242100
...
verify_packaged_notification_defaults() {
  ...
  for expected_default in \
    ...
    '${NOTIFICATION_MOCK_MINI_PROGRAM_ENABLED:false}'; do
...
verify_notification_runtime_disabled() {
  ...
  if grep -Eiq '^(EXTERNAL_NOTIFICATION_ENABLED|EXTERNAL_NOTIFICATION_WORKER_ENABLED|NOTIFICATION_SMS_ALIYUN_ENABLED|NOTIFICATION_MOCK_ENABLED|NOTIFICATION_MOCK_APP_PUSH_ENABLED|NOTIFICATION_MOCK_MINI_PROGRAM_ENABLED)=(true|1)$' <<< "$runtime_environment"; then
    echo "external notification runtime gate must remain disabled" >&2
    return 1
  fi
}
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1083
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1083\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.83 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.83"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success=1')" == "$EXPECTED_PREVIOUS_MIGRATIONS" ]]
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success<>1')" == 0 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version='${EXPECTED_NEW_MIGRATION}'")" == 0 ]]
[[ "$(message_table_count)" == 4 ]]
[[ "$(notification_kernel_table_count)" == 0 ]]
verify_message_schema
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.83.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.83.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.83.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.83.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.83"' "$manifest"
...
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success<>1')" == 0 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version='${EXPECTED_NEW_MIGRATION}' AND success=1")" == 1 ]]
verify_message_schema
verify_notification_kernel
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.83 startup log verification failed" >&2
  exit 1
fi
...
echo "release-success version=$EXPECTED_VERSION backup-before=$BACKUP_BEFORE backup-after=$BACKUP_AFTER build=$EXPECTED_BUILD_ID core-counts=$BEFORE_COUNTS migrations=$EXPECTED_MIGRATIONS message-seeds=15:15 notification-tables=4 budgets=19:off authorizations=0 external-channels=off runtime-gates=off encryption-preserved=yes jar=$EXPECTED_JAR_SHA256"
