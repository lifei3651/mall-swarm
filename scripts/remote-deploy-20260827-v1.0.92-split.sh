...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.92
EXPECTED_VERSION=1.0.92
EXPECTED_PREVIOUS_VERSION=1.0.91
EXPECTED_PREVIOUS_GIT_COMMIT=6f7862a33b01adf4f3233874591d7f2b92fa5fd7
EXPECTED_PREVIOUS_BUILD_ID=20260826-1502-1.0.91
EXPECTED_GIT_COMMIT=9a774ccdc1a4cbf75fc72fd0481a8564a09098be
EXPECTED_BUILD_ID=20260827-1101-1.0.92
EXPECTED_JAR_SHA256=c1574f34303469e1ab681c24c343f3561ec85b4a969022472c765fda56e29a41
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1091
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1091\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.92 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.92"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.92.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.92.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.92.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.92.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.92"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.92 startup log verification failed" >&2
  exit 1
fi
