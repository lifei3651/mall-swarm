...
if (version !== expectedVersion || pkg.version !== expectedVersion || lock.version !== expectedVersion
  ...
  throw new Error('根版本、小程序包版本和锁文件必须全部精确等于 1.0.165')
}
NODE
...
node --check scripts/release-lingqi-165.mjs
node --check scripts/prepare-lingqi-mini-release-165.mjs
node --check scripts/upload-lingqi-mini-165.mjs
node --test scripts/tests/*.test.mjs
python3 - <<'PY'
from pathlib import Path

release = Path('scripts/remote-deploy-20260924-v1.0.165-backend.sh').read_text()
