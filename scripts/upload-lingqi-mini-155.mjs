...
function usage() {
  console.error(`Usage:
  node scripts/upload-lingqi-mini-155.mjs --preflight-only [--port ${DEFAULT_PORT}]
  node scripts/upload-lingqi-mini-155.mjs --authorize-development-upload [--description <text>] [--port ${DEFAULT_PORT}] [--qa-root <repo/document/qa/path>]
...
function resolveQaRoot(root, requested) {
  ...
  const resolved = requested ? path.resolve(requested) : path.join(base, '2026-09-22-mini-155-upload')
