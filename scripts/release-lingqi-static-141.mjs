...
const expected = { version: '1.0.141', edition: 'app-h5-split', application, gitCommit: commit, buildId: '20260914-' + label + '-1.0.141' }
...
walk(dist)
const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi141-' + site + '-package.'))
...
fs.copyFileSync('scripts/remote-deploy-20260914-v1.0.141-static-h5.sh', path.join(stage, 'release.sh'))
...
const remote = cp.execFileSync('ssh', [...opts, host, 'mktemp -d /tmp/lingqimall-' + label + '-141.XXXXXX'], { encoding: 'utf8' }).trim()
if (!new RegExp('^/tmp/lingqimall-' + label + '-141\\.[A-Za-z0-9]+$').test(remote)) throw new Error('Unexpected remote directory')
...
const command = [
  ...
  'export LINGQIMALL_RELEASE_AUTHORIZATION=1.0.141',
  'export LINGQIMALL_STATIC_SITE=' + quote(site),
