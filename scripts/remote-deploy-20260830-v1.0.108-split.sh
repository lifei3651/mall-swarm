...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.108
EXPECTED_VERSION=1.0.108
EXPECTED_PREVIOUS_VERSION=1.0.107
EXPECTED_PREVIOUS_GIT_COMMIT=2ecdc395d1b7560a275c02d2922fea92cb70c87d
EXPECTED_PREVIOUS_BUILD_ID=20260830-1644-1.0.107
EXPECTED_GIT_COMMIT=64c0a8ad6c96012d55689c9438796d141dfa2437
EXPECTED_BUILD_ID=20260830-1916-1.0.108
EXPECTED_JAR_SHA256=70819eae4ee43ff33f15b31704ad197f0cdfdacda78179a392d23a7581ef797f
EXPECTED_PREVIOUS_MIGRATIONS=31
EXPECTED_MIGRATIONS=31
EXPECTED_PRE_RELEASE_CORE_COUNTS=4:2:5:5:9:299001.00:26:6:3
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1108
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1108\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.108 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.108"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.108.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.108.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.108.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.108.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.108"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.108 startup log verification failed" >&2
  exit 1
fi
