...
if (fs.readFileSync('VERSION', 'utf8').trim() !== '1.0.134') throw new Error('Wrong version')
...
const options = ['-i', identity, '-o', 'BatchMode=yes', '-o', 'IdentitiesOnly=yes', '-o', 'ConnectTimeout=12', '-o', 'StrictHostKeyChecking=yes']
const host = 'root@lingqimall.com'
const quote = value => "'" + value.replaceAll("'", "'\\''") + "'"
const remote = cp.execFileSync('ssh', [...options, host, 'mktemp -d /tmp/lingqimall-mini-backend-134.XXXXXX'], { encoding: 'utf8' }).trim()
if (!/^\/tmp\/lingqimall-mini-backend-134\.[A-Za-z0-9]+$/.test(remote)) throw new Error('Unexpected remote staging directory')
const remoteArchive = remote + '.tar.gz'
console.log('release-directory=' + remote)
cp.execFileSync('scp', [...options, archive, host + ':' + remoteArchive], { stdio: 'inherit' })
...
export LINGQIMALL_RELEASE_AUTHORIZATION=1.0.134
