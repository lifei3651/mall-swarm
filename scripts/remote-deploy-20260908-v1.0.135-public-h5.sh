#!/usr/bin/env bash
# Public H5 static only. No backend, team, admin, configuration or database mutation.
set -Eeuo pipefail
umask 077
...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-public-h5-135\.[A-Za-z0-9]+$ ]]
[[ "$EXPECTED_COMMIT" =~ ^[a-f0-9]{40}$ ]]
[[ "$MODE" == --preflight-only || "$MODE" == --authorize-release ]]
[[ "${LINGQIMALL_RELEASE_AUTHORIZATION:-}" == 1.0.135 ]]
...
[[ -d "$TARGET" && ! -L "$TARGET" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == 1.0.134 ]]
...
assert version == {'version':'1.0.135','edition':'app-h5-split','application':'storefront-public','gitCommit':commit,'buildId':'20260908-public-h5-1.0.135'}
print('public-h5-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'public-h5-preflight=passed previous=1.0.115 target=1.0.135 backend=1.0.134'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/public-h5-135.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.shop-135-stage.XXXXXX")
...
with urllib.request.urlopen(base + '/version.json?release=1.0.135', timeout=12) as response: version = json.load(response)
assert version['version'] == '1.0.135' and version['gitCommit'] == sys.argv[1]
with urllib.request.urlopen(base + '/?release=1.0.135', timeout=12) as response: html = response.read().decode()
...
PY
curl -fsS --max-time 12 'https://lingqimall.com/api/shop/products?keyword=%E7%A4%BC%E7%9B%92&pageNum=1&pageSize=1' >/dev/null
[[ "$(protected_hashes)" == "$BEFORE_FILES" ]]
BACKUP_AFTER=$(backup_verify)
[[ "$BACKUP_AFTER" != "$BACKUP_BEFORE" ]]
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "public-h5-release-success version=1.0.135 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-team-admin-config-preserved=yes"
