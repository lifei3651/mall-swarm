...
function usage() {
  console.error(`Usage:
  node scripts/upload-lingqi-mini-158.mjs --preflight-only [--port ${DEFAULT_PORT}]
  node scripts/upload-lingqi-mini-158.mjs --authorize-development-upload \
...
function resolveQaRoot(root, requested) {
  ...
  const resolved = requested ? path.resolve(requested) : path.join(base, '2026-09-23-mini-158-upload')
