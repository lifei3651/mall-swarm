...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.40
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.40
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.40" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.39" ]]
...
admin_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/admin/?release=1.0.40')
shop_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/?release=1.0.40')
...
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$admin_entry?release=1.0.40" >/dev/null
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$shop_entry?release=1.0.40" >/dev/null
...
grep -R -Fq '选中商品暂时无法结算' "$APP_ROOT/nginx/shop/assets"
[[ "$(database_counts)" == "$BEFORE_COUNTS" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.40" ]]
[[ "$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM dms_shop_member WHERE system_account=1 AND user_id IN (-900000000000000001,-900000000000000005)" "$DB_NAME")" == "2" ]]
systemctl is-active --quiet lingqimall-distribution.service
systemctl is-active --quiet nginx
systemctl is-active --quiet mysql
systemctl is-active --quiet redis-server
...
echo "release-success version=1.0.40 backup=$BACKUP_PATH admin_entry=$admin_entry shop_entry=$shop_entry core-counts=$BEFORE_COUNTS"
