...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.107
EXPECTED_VERSION=1.0.107
EXPECTED_PREVIOUS_VERSION=1.0.105
EXPECTED_PREVIOUS_GIT_COMMIT=b51b27e21e09ee3cee33de85b95e1b599fd4b3ce
EXPECTED_PREVIOUS_BUILD_ID=20260830-1533-1.0.105
EXPECTED_GIT_COMMIT=2ecdc395d1b7560a275c02d2922fea92cb70c87d
EXPECTED_BUILD_ID=20260830-1644-1.0.107
EXPECTED_JAR_SHA256=7b3bc4e4a1a7e9eed207cebfeea42e1c93e927ecc56c2cfad8ff86d957522f66
EXPECTED_PREVIOUS_MIGRATIONS=31
EXPECTED_MIGRATIONS=31
EXPECTED_PRE_RELEASE_CORE_COUNTS=3:2:5:5:8:199001.00:26:6:3
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1107
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1107\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.107 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.107"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.107.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.107.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.107.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.107.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.107"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.107 startup log verification failed" >&2
  exit 1
fi
