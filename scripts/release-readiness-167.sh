...
usage() {
  cat <<'EOF'
用法：scripts/release-readiness-167.sh [选项]
...
expected_previous_backend_version = "1.0.165"
expected_previous_backend_jar = "934d345dee88ea7b9d12f8aeff5998e97c1f8d447caff5945e41d25cafbbd7ef"
expected_previous_static_version = "1.0.165"
expected_previous_static_commit = "60a6e5e3fe1255bace2156f0f59089c9a341db0f"
...
if len(repo_migrations) != expected_migration_count or repo_migrations[-1] != expected_last_migration:
    fail("repository migration inventory is not the fixed 1.0.167 set")
...
if sha_file(candidate / "document/db/migrations" / expected_last_migration) != expected_last_migration_sha:
    fail("1.0.167 service-tag migration checksum mismatch")
