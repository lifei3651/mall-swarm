#!/usr/bin/env bash
# shop static only; preserve other sites, backend, configuration and database.
set -Eeuo pipefail
umask 077
...
with tarfile.open(sys.argv[1] + '/files-and-config.tar.gz') as archive:
    for filename in ['index.html', 'version.json']:
        assert archive.extractfile('opt/lingqimall/nginx/shop/' + filename).read() == pathlib.Path(sys.argv[2], filename).read_bytes()
PY
  printf '%s\n' "$backup_path"
}
recover() {
  local status=$?
  trap - EXIT
  if [[ "$status" != 0 && "$MUTATED" == 1 && -d "$ROLLBACK/shop" ]]; then
    if [[ -d "$TARGET" ]]; then mv "$TARGET" "$ROLLBACK/failed-shop"; fi
    mv "$ROLLBACK/shop" "$TARGET"
    echo "public-h5-recovery=previous-static-restored backup=$ROLLBACK"
  fi
  exit "$status"
}
trap recover EXIT
trap 'echo "public-h5-release-check-failed line=$LINENO" >&2' ERR
[[ -d "$TARGET" && ! -L "$TARGET" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == 1.0.139 ]]
...
assert old['version'] == '1.0.138' and old['gitCommit'] == '87dace7f6387266e4be12b637a28716ec69673d8'
with tarfile.open(root + '/shop.tar.gz') as archive:
    names = set()
...
assert version == {'version':'1.0.139','edition':'app-h5-split','application':'storefront-public','gitCommit':commit,'buildId':'20260913-public-h5-1.0.139'}
print('public-h5-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'public-h5-preflight=passed previous=1.0.138 target=1.0.139 backend=1.0.139'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/public-h5-139.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.shop-139-stage.XXXXXX")
tar -xzf "$RELEASE_DIR/shop.tar.gz" -C "$STAGE"
(cd "$STAGE" && sha256sum -c "$RELEASE_DIR/SHOP_SHA256SUMS" >/dev/null)
...
MUTATED=1
mv "$TARGET" "$ROLLBACK/shop"
mv "$STAGE" "$TARGET"
(cd "$TARGET" && sha256sum -c "$RELEASE_DIR/SHOP_SHA256SUMS" >/dev/null)
python3 - "$EXPECTED_COMMIT" <<'PY'
import json, re, sys, urllib.request
base = 'https://lingqimall.com'
with urllib.request.urlopen(base + '/version.json?release=1.0.139', timeout=12) as response: version = json.load(response)
assert version['version'] == '1.0.139' and version['gitCommit'] == sys.argv[1]
with urllib.request.urlopen(base + '/?release=1.0.139', timeout=12) as response: html = response.read().decode()
entries = re.findall(r'(?:src|href)="(/assets/[^" ]+)"', html)
assert entries and '<div id="app">' in html
for entry in entries:
    with urllib.request.urlopen('https://lingqimall.com' + entry, timeout=12) as response: assert response.status == 200
print('public-h5-version-html-entry-assets=passed')
PY
[[ "$(protected_hashes)" == "$BEFORE_FILES" ]]
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "public-h5-release-success version=1.0.139 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
