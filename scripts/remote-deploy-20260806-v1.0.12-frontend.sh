...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.12
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.12
BACKUP_PATH=""
MUTATED=0
...
rollback() {
  ...
  if [[ "$code" != 0 && "$MUTATED" == 1 ]]; then
    echo "frontend release failed; restoring previous web assets and version" >&2
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.12" ]]
...
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.12" ]]
systemctl is-active --quiet nginx
systemctl is-active --quiet mysql
systemctl is-active --quiet redis-server
...
echo "release-success version=1.0.12 backup=$BACKUP_PATH shop_entry=$shop_entry admin_entry=$admin_entry"
