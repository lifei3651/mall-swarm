...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.109
EXPECTED_VERSION=1.0.109
EXPECTED_PREVIOUS_VERSION=1.0.108
EXPECTED_PREVIOUS_GIT_COMMIT=64c0a8ad6c96012d55689c9438796d141dfa2437
EXPECTED_PREVIOUS_BUILD_ID=20260830-1916-1.0.108
EXPECTED_GIT_COMMIT=c3f3e014f01fb252fa15d4971382c19e591441ee
EXPECTED_BUILD_ID=20260901-0914-1.0.109
EXPECTED_JAR_SHA256=3a181a08b709bff79b0d5891c3618eaf4895b865e1b14c22234e766aab44b3c6
EXPECTED_PREVIOUS_MIGRATIONS=31
EXPECTED_MIGRATIONS=31
EXPECTED_PRE_RELEASE_CORE_COUNTS=4:2:6:5:10:299000.99:26:6:3
...
verify_previous_release_schema_for() {
  local database=$1
...
wechat_mini_program_schema_counts_for() {
  local database=$1
  mysql_root -NBe "SELECT CONCAT(
    (SELECT COUNT(*) FROM information_schema.tables
      WHERE table_schema='${database}' AND table_name='dms_wechat_mini_program_identity'), ':',
    (SELECT COUNT(*) FROM information_schema.columns
      WHERE table_schema='${database}' AND table_name='dms_wechat_mini_program_identity'), ':',
    (SELECT COUNT(DISTINCT index_name) FROM information_schema.statistics
      WHERE table_schema='${database}' AND table_name='dms_wechat_mini_program_identity'), ':',
    (SELECT COUNT(*) FROM information_schema.referential_constraints
      WHERE constraint_schema='${database}' AND table_name='dms_wechat_mini_program_identity')
  )"
}

verify_release_additions_absent_for() {
  local database=$1
  verify_previous_release_schema_for "$database"
  [[ "$(wechat_mini_program_schema_counts_for "$database")" == 0:0:0:0 ]]
}

verify_release_schema_for() {
  local database=$1
  verify_previous_release_schema_for "$database"
  [[ "$(wechat_mini_program_schema_counts_for "$database")" == 1:16:5:0 ]]
  [[ "$(mysql --protocol=socket -uroot "$database" -NBe 'SELECT COUNT(*) FROM dms_wechat_mini_program_identity')" == 0 ]]
}
...
verify_packaged_notification_defaults() {
  ...
  for expected_default in \
    ...
    '${WECHAT_MINI_PROGRAM_PHONE_AUTH_ENABLED:false}' \
  ...
  for expected_default in \
    ...
    '${SHOP_SERVICE_TICKET_FIRST_RESPONSE_HOURS:24}' \
    '${ADMIN_SESSION_HOURS:168}' \
    '${ADMIN_MAX_ACTIVE_SESSIONS:3}' \
    '${SHOP_SESSION_DAYS:30}' \
    '${SHOP_MAX_ACTIVE_SESSIONS:5}'; do
...
verify_notification_runtime_disabled() {
  ...
  if grep -Eiq '^(EXTERNAL_NOTIFICATION_ENABLED|EXTERNAL_NOTIFICATION_WORKER_ENABLED|NOTIFICATION_SMS_ALIYUN_ENABLED|NOTIFICATION_MOCK_ENABLED|NOTIFICATION_MOCK_APP_PUSH_ENABLED|NOTIFICATION_MOCK_MINI_PROGRAM_ENABLED|WECHAT_MINI_PROGRAM_ENABLED|WECHAT_MINI_PROGRAM_PHONE_AUTH_ENABLED|WECHAT_PAY_ENABLED|SHOP_REAL_NAME_ENABLED)=(true|1)$' <<< "$runtime_environment"; then
    echo "external notification, WeChat mini-program/payment and real-name runtime gates must remain disabled" >&2
    return 1
  fi
}
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1109
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1109\`" >/dev/null 2>&1 || true' EXIT
    ...
    verify_release_schema_for "$verify_db"
  )
}
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.109 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.109"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.109.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.109.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.109.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.109.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.109"' "$manifest"
...
grep -R -Fq '账号或密码错误' "$NEW_ADMIN/assets"
! grep -R -Fq '前往平台总后台' "$NEW_ADMIN/assets"
! grep -R -Fq '前往商家后台' "$NEW_ADMIN/assets"
grep -R -Fq '购买账号' "$NEW_ADMIN/assets"
grep -R -Fq '已发货' "$NEW_ADMIN/assets"
grep -R -Fq '查看审计详情' "$NEW_ADMIN/assets"
grep -R -Fq '确认售后处理' "$NEW_ADMIN/assets"
grep -R -Fq '确认提交' "$NEW_ADMIN/assets"
grep -R -Fq '返回检查' "$NEW_ADMIN/assets"
grep -R -Fq '手动输入购买数量' "$NEW_SHOP/assets"
grep -R -Fq '请选择快递公司' "$NEW_SHOP/assets"
grep -R -Fq '提交退货物流' "$NEW_SHOP/assets"
grep -R -Fq '可与短信验证码任意顺序填写' "$NEW_SHOP/assets" "$NEW_TEAM/assets"
! grep -R -Fq '获取短信验证码需要填写图形验证码' "$NEW_SHOP/assets" "$NEW_TEAM/assets"
! grep -R -Fq '请先输入图形验证码' "$NEW_SHOP/assets" "$NEW_TEAM/assets"
grep -R -Fq 'shop-mobile-viewport-locked' "$NEW_SHOP/assets" "$NEW_TEAM/assets"
...
grep -R -Fq '账号或密码错误' "$APP_ROOT/nginx/admin/assets"
! grep -R -Fq '前往平台总后台' "$APP_ROOT/nginx/admin/assets"
! grep -R -Fq '前往商家后台' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '购买账号' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '已发货' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '查看审计详情' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '确认售后处理' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '确认提交' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '返回检查' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '手动输入购买数量' "$APP_ROOT/nginx/shop/assets"
grep -R -Fq '请选择快递公司' "$APP_ROOT/nginx/shop/assets"
grep -R -Fq '提交退货物流' "$APP_ROOT/nginx/shop/assets"
grep -R -Fq '可与短信验证码任意顺序填写' "$APP_ROOT/nginx/shop/assets" "$APP_ROOT/nginx/team/assets"
! grep -R -Fq '获取短信验证码需要填写图形验证码' "$APP_ROOT/nginx/shop/assets" "$APP_ROOT/nginx/team/assets"
! grep -R -Fq '请先输入图形验证码' "$APP_ROOT/nginx/shop/assets" "$APP_ROOT/nginx/team/assets"
grep -R -Fq 'shop-mobile-viewport-locked' "$APP_ROOT/nginx/shop/assets" "$APP_ROOT/nginx/team/assets"
...
grep -q '"requestId"' <<< "$product_response"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.109 startup log verification failed" >&2
  exit 1
fi
