...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.43
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.43
...
[[ -s "$RELEASE_DIR/20260810_bonus_task_concurrency_guard.sql" ]]
[[ -s "$RELEASE_DIR/VERSION" ]]
[[ -s "$RELEASE_DIR/SHA256SUMS" ]]
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.43" ]]
...
# 唯一键执行前先拒绝历史重复数据；不自动删除或合并任何奖金记录。
duplicate_groups=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe \
  "SELECT COUNT(*) FROM (SELECT order_id FROM dms_bonus_calculation_task GROUP BY order_id HAVING COUNT(*) > 1) duplicates" "$DB_NAME")
[[ "$duplicate_groups" == "0" ]]
MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" "$DB_NAME" \
  < "$RELEASE_DIR/20260810_bonus_task_concurrency_guard.sql"
bonus_index_count=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe \
  "SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='dms_bonus_calculation_task' AND index_name='uk_bonus_task_order'" "$DB_NAME")
[[ "$bonus_index_count" == "1" ]]
...
admin_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/admin/?release=1.0.43')
shop_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/?release=1.0.43')
...
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$admin_entry?release=1.0.43" >/dev/null
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$shop_entry?release=1.0.43" >/dev/null
...
grep -R -Fq '待售后' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '库存不足，最多可购买' "$APP_ROOT/nginx/shop/assets"
[[ "$(database_counts)" == "$BEFORE_COUNTS" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.43" ]]
systemctl is-active --quiet lingqimall-distribution.service
systemctl is-active --quiet nginx
systemctl is-active --quiet mysql
systemctl is-active --quiet redis-server
...
echo "release-success version=1.0.43 backup=$BACKUP_PATH admin_entry=$admin_entry shop_entry=$shop_entry core-counts=$BEFORE_COUNTS"
