#!/usr/bin/env bash
# Deploy exactly one immutable 1.0.155 static surface from the unified closure candidate.
set -Eeuo pipefail
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-155\.[A-Za-z0-9]+$ ]] || fail "invalid candidate directory"
...
case "$SITE" in
  admin)
    ...
    PREVIOUS_BUILD_ID=20260921-closure-1.0.154
    ;;
  shop)
    LABEL=public-h5
...
assert manifest['version'] == '1.0.155'
...
assert manifest['previousStaticVersion'] == '1.0.154'
assert manifest['previousStaticCommit'] == 'a8f86f2ed3123c21082e7404d372812e11dae790'
...
assert candidate == {
    'version':'1.0.155', 'edition':'app-h5-split', 'application':application,
    'gitCommit':commit, 'buildId':build_id,
}
...
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "$EXPECTED_VERSION" ]] || fail "backend version is not 1.0.155"
...
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/$LABEL-155.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.$SITE-155-stage.XXXXXX")
...
query = '?release=1.0.155-' + commit[:12]
