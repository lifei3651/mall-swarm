#!/usr/bin/env bash
# 1.0.165 closure backend release. No database migration may be applied.
set -Eeuo pipefail
...
EXPECTED_PREVIOUS_JAR_SHA=cbd249426c1dfcdad4de7dbf65f9780486b47a9807e35ae3eb27fb8252f1ae1f
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-165\.[A-Za-z0-9]+$ ]] || fail "invalid release directory"
...
verify_unauthorized_contract() {
  local body status
  body=$(mktemp /tmp/lingqimall-closure-165-unauthorized.XXXXXX)
...
atomic_install() {
  ...
  temporary=$(mktemp "$(dirname "$destination")/.release-165.$(basename "$destination").XXXXXX") || return 1
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/closure-backend-165.XXXXXX)
...
VERIFY_DB=m165v_$(date +%Y%m%d%H%M%S)
[[ "$VERIFY_DB" =~ ^m165v_[0-9]{14}$ ]] || fail "invalid verification database"
