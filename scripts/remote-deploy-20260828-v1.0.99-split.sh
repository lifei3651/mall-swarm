...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.99
EXPECTED_VERSION=1.0.99
EXPECTED_PREVIOUS_VERSION=1.0.98
EXPECTED_PREVIOUS_GIT_COMMIT=00a30c2fa047847b19e0a14b3d0837a9c785b2f6
EXPECTED_PREVIOUS_BUILD_ID=20260828-1501-1.0.98
EXPECTED_GIT_COMMIT=5cbf46ea5d0b0c464010fdafa74c1a779f4d7bf5
EXPECTED_BUILD_ID=20260828-1653-1.0.99
EXPECTED_JAR_SHA256=61a177c653c44310d92c82f2e0d1d98c32fc7dbed93ff84e6a76d8102aa0f092
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.99 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.99"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.99.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.99.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.99.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.99.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.99"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.99 startup log verification failed" >&2
  exit 1
fi
