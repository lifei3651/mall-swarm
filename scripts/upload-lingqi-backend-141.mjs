...
if (fs.readFileSync('VERSION', 'utf8').trim() !== '1.0.141') throw new Error('Wrong version')
...
const options = ['-i', '/Users/minmatemp/.ssh/lingqi_server_ed25519', '-o', 'BatchMode=yes', '-o', 'IdentitiesOnly=yes', '-o', 'ConnectTimeout=12', '-o', 'StrictHostKeyChecking=yes', '-o', 'ServerAliveInterval=10', '-o', 'ServerAliveCountMax=3']
const host = 'root@lingqimall.com'
const quote = value => "'" + value.replaceAll("'", "'\\''") + "'"
const remote = cp.execFileSync('ssh', [...options, host, 'mktemp -d /tmp/lingqimall-mini-backend-141.XXXXXX'], { encoding: 'utf8' }).trim()
if (!/^\/tmp\/lingqimall-mini-backend-141\.[A-Za-z0-9]+$/.test(remote)) throw new Error('Unexpected remote staging directory')
