...
const PROJECT = '/Users/minmatemp/Documents/mall-swarm-app-h5/dist/wechat-mini-program'
...
function usage() {
  console.error(`Usage:
  node scripts/upload-lingqi-mini-153.mjs --preflight-only [--port ${DEFAULT_PORT}]
  node scripts/upload-lingqi-mini-153.mjs --authorize-development-upload [--description <text>] [--port ${DEFAULT_PORT}] [--qa-root <repo/document/qa/path>]
...
function resolveQaRoot(root, requested) {
  ...
  const resolved = requested ? path.resolve(requested) : path.join(base, '2026-09-21-mini-153-upload')
