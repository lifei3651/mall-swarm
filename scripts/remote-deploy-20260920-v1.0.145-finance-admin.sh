#!/usr/bin/env bash
# Admin static-only finance audit release. Preserve backend, database, other sites and configuration.
set -Eeuo pipefail
umask 077
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-finance-admin-145\.[A-Za-z0-9]+$ ]]
[[ "$EXPECTED_COMMIT" =~ ^[a-f0-9]{40}$ ]]
[[ "$MODE" == --preflight-only || "$MODE" == --authorize-release ]]
[[ "${LINGQIMALL_RELEASE_AUTHORIZATION:-}" == 1.0.145 ]]
...
[[ -d "$TARGET" && ! -L "$TARGET" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == 1.0.145 ]]
...
assert version == {'version':'1.0.145','edition':'app-h5-split','application':'admin','gitCommit':commit,'buildId':'20260920-admin-1.0.145'}
print('admin-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'admin-preflight=passed previous=1.0.139 target=1.0.145 backend=1.0.145'
[[ "$MODE" != --preflight-only ]] || exit 0

BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/finance-admin-145.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.finance-admin-145-stage.XXXXXX")
...
with urllib.request.urlopen(base + '/version.json?release=1.0.145', timeout=12) as response: version = json.load(response)
assert version['version'] == '1.0.145' and version['gitCommit'] == sys.argv[1]
with urllib.request.urlopen(base + '/?release=1.0.145', timeout=12) as response: html = response.read().decode()
...
PY
grep -R -Fq '利润状态' "$TARGET/assets"
grep -R -Fq '已实现利润' "$TARGET/assets"
grep -R -Fq '预计利润' "$TARGET/assets"
[[ "$(protected_hashes)" == "$BEFORE_FILES" ]]
BACKUP_AFTER=$(backup_verify)
[[ "$BACKUP_AFTER" != "$BACKUP_BEFORE" ]]
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "admin-release-success version=1.0.145 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
