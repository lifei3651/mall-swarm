...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.20-admin
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.20-admin
BACKUP_PATH=""
MUTATED=0
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.20" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.19" ]]
...
echo "release-success version=1.0.20 backup=$BACKUP_PATH admin_entry=$admin_entry"
