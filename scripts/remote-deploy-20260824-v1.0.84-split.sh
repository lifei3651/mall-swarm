...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.84
EXPECTED_VERSION=1.0.84
EXPECTED_PREVIOUS_VERSION=1.0.82
EXPECTED_PREVIOUS_GIT_COMMIT=73fab35b2cef0f35507a3b35aaa67951c5845e38
EXPECTED_PREVIOUS_BUILD_ID=20260824-1233-1.0.82
EXPECTED_GIT_COMMIT=ef97831c175f89daff0de8eeb895ff575c1a3385
EXPECTED_BUILD_ID=20260824-1645-1.0.84
EXPECTED_JAR_SHA256=f80481cc0f51ff5de144ca31b72120eda226e5bbe647398a485d0b6a30e5d531
EXPECTED_PREVIOUS_MIGRATIONS=22
EXPECTED_MIGRATIONS=24
EXPECTED_NOTIFICATION_MIGRATION=202608242100
EXPECTED_REAL_NAME_MIGRATION=202608242330
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1084
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1084\`" >/dev/null 2>&1 || true' EXIT
    ...
    [[ "$(mysql_root -NBe "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='${verify_db}' AND table_name IN ('dms_message_delivery_attempt','dms_message_cost_budget','dms_message_recipient_authorization','dms_message_delivery_receipt')")" == 0 ]]
    [[ "$(mysql_root -NBe "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='${verify_db}' AND table_name IN ('dms_member_real_name','dms_member_real_name_attempt')")" == 0 ]]
    [[ "$(mysql_root -NBe "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema='${verify_db}' AND table_name='dms_shop_member_session' AND column_name='surface'")" == 0 ]]
...
restore_database_from_backup() {
  ...
  [[ "$(message_table_count)" == 4 ]]
  [[ "$(notification_kernel_table_count)" == 0 ]]
  [[ "$(real_name_table_count)" == 0 ]]
  [[ "$(session_surface_column_count)" == 0 ]]
  verify_message_schema
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.84 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.84"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success=1')" == "$EXPECTED_PREVIOUS_MIGRATIONS" ]]
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success<>1')" == 0 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version IN ('${EXPECTED_NOTIFICATION_MIGRATION}','${EXPECTED_REAL_NAME_MIGRATION}')")" == 0 ]]
[[ "$(message_table_count)" == 4 ]]
[[ "$(notification_kernel_table_count)" == 0 ]]
[[ "$(real_name_table_count)" == 0 ]]
[[ "$(session_surface_column_count)" == 0 ]]
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.84.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.84.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.84.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.84.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.84"' "$manifest"
...
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success<>1')" == 0 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version='${EXPECTED_NOTIFICATION_MIGRATION}' AND success=1")" == 1 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version='${EXPECTED_REAL_NAME_MIGRATION}' AND success=1")" == 1 ]]
verify_message_schema
verify_notification_kernel
verify_real_name_schema
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.84 startup log verification failed" >&2
  exit 1
fi
...
echo "release-success version=$EXPECTED_VERSION backup-before=$BACKUP_BEFORE backup-after=$BACKUP_AFTER build=$EXPECTED_BUILD_ID core-counts=$BEFORE_COUNTS migrations=$EXPECTED_MIGRATIONS message-seeds=15:15 notification-tables=4 budgets=19:off authorizations=0 external-channels=off real-name=off real-name-records=0 session-surface=on transfer=integrated-only encryption-preserved=yes jar=$EXPECTED_JAR_SHA256"
