...
function usage() {
  console.error(`Usage:
  node scripts/upload-lingqi-mini-154.mjs --preflight-only [--port ${DEFAULT_PORT}]
  node scripts/upload-lingqi-mini-154.mjs --authorize-development-upload [--description <text>] [--port ${DEFAULT_PORT}] [--qa-root <repo/document/qa/path>]
...
function resolveQaRoot(root, requested) {
  ...
  const resolved = requested ? path.resolve(requested) : path.join(base, '2026-09-21-mini-154-upload')
