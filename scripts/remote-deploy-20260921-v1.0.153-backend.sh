#!/usr/bin/env bash
# 1.0.153 closure backend release. No schema migration is allowed in this release.
set -Eeuo pipefail
shopt -s inherit_errexit
umask 077

EXPECTED_VERSION=1.0.153
EXPECTED_PREVIOUS_VERSION=1.0.152
EXPECTED_PREVIOUS_JAR_SHA=5acbcd7b43c2b41a6aa6db685fbdbffa0fb39d17a01ae694fa3d5c29b4273893
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-153\.[A-Za-z0-9]+$ ]] || fail "invalid release directory"
...
verify_unauthorized_contract() {
  local body status
  body=$(mktemp /tmp/lingqimall-closure-153-unauthorized.XXXXXX)
...
atomic_install() {
  ...
  temporary=$(mktemp "$(dirname "$destination")/.release-153.$(basename "$destination").XXXXXX") || return 1
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/closure-backend-153.XXXXXX)
