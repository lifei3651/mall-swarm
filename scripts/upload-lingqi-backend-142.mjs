...
if (fs.readFileSync('VERSION', 'utf8').trim() !== '1.0.142') throw new Error('Wrong version')
...
const remote = cp.execFileSync('ssh', [...options, host, 'mktemp -d /tmp/lingqimall-pay-hotfix-142.XXXXXX'], { encoding: 'utf8' }).trim()
if (!/^\/tmp\/lingqimall-pay-hotfix-142\.[A-Za-z0-9]+$/.test(remote)) throw new Error('Unexpected remote staging directory')
...
export LINGQIMALL_RELEASE_AUTHORIZATION=1.0.142
