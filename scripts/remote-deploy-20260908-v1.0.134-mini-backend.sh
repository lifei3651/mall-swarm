...
EXPECTED_PREVIOUS_VERSION=1.0.132
OLD_JAR_SHA=e23979000f84f4636f8844b04972723e20139306bcd560e9485b150ee7768975
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-mini-backend-134\.[A-Za-z0-9]+$ ]]
...
with open(root + '/RELEASE_MANIFEST.json') as f:
    m = json.load(f)
assert m['version'] == '1.0.134' and m['scope'] == 'backend-only'
...
assert m['buildId'] == '20260908-mini-backend-1.0.134'
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/mini-backend-134.XXXXXX)
...
VERIFY_DB=mall_distribution_release_verify_134_$(date +%Y%m%d%H%M%S)
[[ "$VERIFY_DB" =~ ^mall_distribution_release_verify_134_[0-9]{14}$ ]]
