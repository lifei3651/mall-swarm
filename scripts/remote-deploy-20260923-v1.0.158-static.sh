#!/usr/bin/env bash
# Deploy exactly one immutable 1.0.158 static surface from the unified closure candidate.
set -Eeuo pipefail
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-158\.[A-Za-z0-9]+$ ]] || fail "invalid candidate directory"
...
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "$EXPECTED_VERSION" ]] || fail "backend version is not 1.0.158"
...
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/$LABEL-158.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.$SITE-158-stage.XXXXXX")
...
query = '?release=1.0.158-' + commit[:12]
