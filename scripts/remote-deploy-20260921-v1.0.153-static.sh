#!/usr/bin/env bash
# Deploy exactly one immutable 1.0.153 static surface from the unified closure candidate.
set -Eeuo pipefail
shopt -s inherit_errexit
umask 077

EXPECTED_VERSION=1.0.153
EXPECTED_PREVIOUS_VERSION=1.0.151
EXPECTED_PREVIOUS_COMMIT=e25c760c4428fcc65956847adbf8bbb252f76db3
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-153\.[A-Za-z0-9]+$ ]] || fail "invalid candidate directory"
...
case "$SITE" in
  admin)
    ...
    PREVIOUS_BUILD_ID=20260920-admin-1.0.151
    ;;
  shop)
    LABEL=public-h5
    APPLICATION=storefront-public
    BASE=https://lingqimall.com
    PREVIOUS_BUILD_ID=20260920-public-h5-1.0.151
    ;;
  team)
    LABEL=team-h5
    APPLICATION=team-h5
    BASE=https://www.lingqimall.com
    PREVIOUS_BUILD_ID=20260920-team-h5-1.0.151
    ;;
esac
...
expected = {
    'version':'1.0.151', 'edition':'app-h5-split', 'application':application,
...
mapfile -t IDENTITY < <(python3 - "$RELEASE_MANIFEST" "$EXPECTED_COMMIT" "$SITE" "$ARCHIVE" "$FILE_MANIFEST" <<'PY'
import hashlib, json, re, sys
manifest_path, commit, site, archive, checksums = sys.argv[1:]
with open(manifest_path, encoding='utf-8') as stream:
    manifest = json.load(stream)
assert manifest['version'] == '1.0.153'
assert manifest['scope'] == 'mall-closure-candidate'
assert manifest['gitCommit'] == commit
assert isinstance(manifest['buildId'], str) and re.fullmatch(r'[A-Za-z0-9._-]{8,128}', manifest['buildId'])
assert re.fullmatch(r'[a-f0-9]{64}', manifest['jarSha256'])
assert manifest['previousVersion'] == '1.0.152'
assert manifest['previousJarSha256'] == '5acbcd7b43c2b41a6aa6db685fbdbffa0fb39d17a01ae694fa3d5c29b4273893'
assert manifest['previousStaticVersion'] == '1.0.151'
assert manifest['previousStaticCommit'] == 'e25c760c4428fcc65956847adbf8bbb252f76db3'
...
[[ "${#IDENTITY[@]}" == 2 ]] || fail "invalid release identity"
EXPECTED_BACKEND_JAR_SHA=${IDENTITY[0]}
EXPECTED_BUILD_ID=${IDENTITY[1]}
...
assert candidate == {
    'version':'1.0.153', 'edition':'app-h5-split', 'application':application,
    'gitCommit':commit, 'buildId':build_id,
}
...
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "$EXPECTED_VERSION" ]] || fail "backend version is not 1.0.153"
...
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/$LABEL-153.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.$SITE-153-stage.XXXXXX")
...
query = '?release=1.0.153-' + commit[:12]
