...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.44
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.44
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.44" ]]
...
admin_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/admin/?release=1.0.44')
shop_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/?release=1.0.44')
...
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$admin_entry?release=1.0.44" >/dev/null
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$shop_entry?release=1.0.44" >/dev/null
...
grep -R -Fq '解除支付密码锁定' "$APP_ROOT/nginx/admin/assets"
[[ "$(database_counts)" == "$BEFORE_COUNTS" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.44" ]]
systemctl is-active --quiet lingqimall-distribution.service
systemctl is-active --quiet nginx
systemctl is-active --quiet mysql
systemctl is-active --quiet redis-server
...
echo "release-success version=1.0.44 backup=$BACKUP_PATH admin_entry=$admin_entry shop_entry=$shop_entry core-counts=$BEFORE_COUNTS"
