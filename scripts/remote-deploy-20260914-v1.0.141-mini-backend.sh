...
umask 077
EXPECTED_VERSION=1.0.141
EXPECTED_PREVIOUS_VERSION=1.0.139
OLD_JAR_SHA=06aae30e8580b6edecfbef080fbd1cf0a6f25376067f8ec249b84028f497d198
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-mini-backend-141\.[A-Za-z0-9]+$ ]]
...
for entry in entries:
    ...
    if danger and value.lower() in (b'true', b'1'): raise SystemExit('unexpected enabled runtime flag: ' + key.decode(errors='replace'))
...
assert m['version'] == '1.0.141' and m['scope'] == 'backend-only'
assert m['gitCommit'] == os.environ['LINGQIMALL_RELEASE_COMMIT']
assert m['buildId'] == '20260914-mini-backend-1.0.141' and m['databaseMigrations'] == []
...
with zipfile.ZipFile(root + '/mall-distribution.jar') as z:
    assert 'BOOT-INF/classes/com/macro/mall/distribution/service/ShopMediaStorageService.class' in z.namelist()
    assert 'BOOT-INF/classes/mapper/DmsShopProductMapper.xml' in z.namelist()
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/mini-backend-141.XXXXXX)
...
VERIFY_DB=mall_distribution_release_verify_141_$(date +%Y%m%d%H%M%S)
...
MUTATED=0
echo "release-success version=$EXPECTED_VERSION backup-before=$BACKUP_BEFORE backup-after=$BACKUP_AFTER rollback=$ROLLBACK_DIR migrations=36:36 no-data-migrations=yes configuration-and-static-sites-preserved=yes payment-payout-unchanged=yes"
