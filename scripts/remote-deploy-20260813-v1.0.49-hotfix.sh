...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.49
EXPECTED_VERSION=1.0.49
EXPECTED_PREVIOUS_VERSION=1.0.48
EXPECTED_GIT_COMMIT=30007b69710e0a8b5234b464ce607a8f2f876c99
EXPECTED_BUILD_ID=20260813-0843-1.0.49
EXPECTED_JAR_SHA256=c5f199a7bba9ebdf9b56d9ac3fc97ae5da900aff6a3b5e8c9102a029e0a9963a
...
rollback() {
  ...
  if [[ "$code" != 0 && "$MUTATED" == 1 ]]; then
    echo "hotfix failed; restoring 1.0.48 application files" >&2
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.49.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.49.XXXXXX")"
...
for manifest in "$NEW_ADMIN/version.json" "$NEW_SHOP/version.json"; do
  grep -q '"version": "1.0.49"' "$manifest"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.49.XXXXXX)"
...
# 旧版本的 5 秒定时任务可能在停止过程中再写一条日志；必须等旧进程
# 完全退出后再记录字节起点，避免把 1.0.48 的旧错误误判为 1.0.49。
