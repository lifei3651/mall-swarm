...
if (site !== 'shop' || !config || !['--package-only', '--authorize-release'].includes(mode)) throw new Error('Invalid explicit scope/mode')
...
const expected = { version: '1.0.138', edition: 'app-h5-split', application, gitCommit: commit, buildId: '20260911-' + label + '-1.0.138' }
...
walk(dist)
const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi138-' + site + '-package.'))
...
fs.copyFileSync('scripts/remote-deploy-20260911-v1.0.138-' + label + '.sh', path.join(stage, 'release.sh'))
...
const remote = cp.execFileSync('ssh', [...opts, host, 'mktemp -d /tmp/lingqimall-' + label + '-138.XXXXXX'], { encoding: 'utf8' }).trim()
if (!new RegExp('^/tmp/lingqimall-' + label + '-138\\.[A-Za-z0-9]+$').test(remote)) throw new Error('Unexpected remote directory')
...
const command = [
  ...
  'export LINGQIMALL_RELEASE_AUTHORIZATION=1.0.138',
