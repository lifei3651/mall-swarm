...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.98
EXPECTED_VERSION=1.0.98
EXPECTED_PREVIOUS_VERSION=1.0.97
EXPECTED_PREVIOUS_GIT_COMMIT=a95ac56272e797c0ae3ab90a669a548ccbffe7e6
EXPECTED_PREVIOUS_BUILD_ID=20260828-1428-1.0.97
EXPECTED_GIT_COMMIT=00a30c2fa047847b19e0a14b3d0837a9c785b2f6
EXPECTED_BUILD_ID=20260828-1501-1.0.98
EXPECTED_JAR_SHA256=903afbc84b63c4095e3fcbe6d6e4b32d6f3ea61bc4c7f13389e26af0d10bcad7
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.98 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.98"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.98.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.98.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.98.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.98.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.98"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.98 startup log verification failed" >&2
  exit 1
fi
