...
function usage() {
  console.error(`Usage:
  node scripts/upload-lingqi-mini-165.mjs --preflight-only [--port ${DEFAULT_PORT}]
  node scripts/upload-lingqi-mini-165.mjs --authorize-development-upload \
...
function resolveQaRoot(root, requested) {
  ...
  const resolved = requested ? path.resolve(requested) : path.join(base, '2026-09-24-mini-165-upload')
