...
umask 077
EXPECTED_VERSION=1.0.137
EXPECTED_PREVIOUS_VERSION=1.0.136
OLD_JAR_SHA=8e66e7ef3f2e1d924d92cdd7512a4f5b697d8392eda7b569dc0acbe58bb28539
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-mini-backend-137\.[A-Za-z0-9]+$ ]]
...
with open(root + '/RELEASE_MANIFEST.json') as f:
    m = json.load(f)
assert m['version'] == '1.0.137' and m['scope'] == 'backend-only'
...
assert m['buildId'] == '20260909-mini-backend-1.0.137'
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/mini-backend-137.XXXXXX)
...
VERIFY_DB=mall_distribution_release_verify_137_$(date +%Y%m%d%H%M%S)
[[ "$VERIFY_DB" =~ ^mall_distribution_release_verify_137_[0-9]{14}$ ]]
