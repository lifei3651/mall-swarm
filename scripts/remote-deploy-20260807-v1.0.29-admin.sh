...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.29-admin
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.29-admin
BACKUP_PATH=""
MUTATED=0
...
rollback() {
  ...
  if [[ "$code" != 0 && "$MUTATED" == 1 ]]; then
    echo "admin patch failed; restoring previous admin assets and version" >&2
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.29" ]]
...
admin_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/admin/?release=1.0.29')
admin_entry=$(grep -o '/admin/assets/index-[^" ]*\.js' <<< "$admin_html" | head -1)
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$admin_entry?release=1.0.29" >/dev/null
grep -R -Fq '后台退款' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '当前登录管理员' "$APP_ROOT/nginx/admin/assets" || true
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.29" ]]

MUTATED=0
trap - EXIT
rm -rf -- "$RELEASE_DIR" "$ROLLBACK_DIR"
echo "release-success version=1.0.29 backup=$BACKUP_PATH admin_entry=$admin_entry"
