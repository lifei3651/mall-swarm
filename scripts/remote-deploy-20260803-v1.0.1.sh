...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.1
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.1
...
EXPECTED_JAR=ac15357f8aa9d1927947ac36a39acfad08b5cb71a4ebb0b8712c2144a19b39a0
EXPECTED_ADMIN=37eaf94f28bb74ef029286b8c29331d7048a1f3e216649c439473fe7cba4735d
EXPECTED_SHOP=42cbd629572e90e87b1aea778acc7a1c9d49ab036f26d078bff28c31fc087743
EXPECTED_ADMIN_INDEX=455225c9e8a50b2ea44f472baaca5e973bc75956f74506a5fbb9ab6c65781d45
...
grep -R -Fq '格式正确，请通过短信验证码确认号码可用' "$APP_ROOT/nginx/shop/assets"
grep -R -Fq '请完善标红的必填信息' "$APP_ROOT/nginx/shop/assets"
...
legal_json=$(curl -fsS --max-time 12 https://lingqimall.com/api/shop/legal-config)
...
# 发布成功后清理临时回滚副本和上传包；标准全量备份按备份策略保留。
MUTATED=0
trap - EXIT
