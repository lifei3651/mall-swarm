...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-public-h5-140\.[A-Za-z0-9]+$ ]]
[[ "$EXPECTED_COMMIT" =~ ^[a-f0-9]{40}$ ]]
[[ "$MODE" == --preflight-only || "$MODE" == --authorize-release ]]
[[ "${LINGQIMALL_RELEASE_AUTHORIZATION:-}" == 1.0.140 ]]
...
assert old['version'] == '1.0.139' and old['gitCommit'] == 'e6a11c3df56d4be0a7181f67457584ce6218c59f'
...
assert version == {'version':'1.0.140','edition':'app-h5-split','application':'storefront-public','gitCommit':commit,'buildId':'20260913-public-h5-1.0.140'}
print('public-h5-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'public-h5-preflight=passed previous=1.0.139 target=1.0.140 backend=1.0.139'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/public-h5-140.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.shop-140-stage.XXXXXX")
...
with urllib.request.urlopen(base + '/version.json?release=1.0.140', timeout=12) as response: version = json.load(response)
assert version['version'] == '1.0.140' and version['gitCommit'] == sys.argv[1]
with urllib.request.urlopen(base + '/?release=1.0.140', timeout=12) as response: html = response.read().decode()
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "public-h5-release-success version=1.0.140 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
