...
const BUILD_ID = '20260924-closure-1.0.165'
const PREVIOUS_BACKEND_VERSION = '1.0.158'
const PREVIOUS_BACKEND_JAR_SHA256 = 'cbd249426c1dfcdad4de7dbf65f9780486b47a9807e35ae3eb27fb8252f1ae1f'
const PREVIOUS_STATIC_VERSION = '1.0.158'
const PREVIOUS_STATIC_COMMIT = 'fcadf4099861a0a5359b12761cbf232c160635a7'
...
if (process.argv.length !== 3 || process.argv[2] !== '--package-only') {
  throw new Error('Usage: node scripts/release-lingqi-165.mjs --package-only')
}
...
if (version !== VERSION || miniPackage.version !== VERSION || miniLock.version !== VERSION
  ...
  throw new Error('Root and mini-program versions must all equal 1.0.165')
}
...
const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-165-unified-candidate.'))
...
copy('scripts/remote-deploy-20260924-v1.0.165-backend.sh', path.join(stage, 'release-backend.sh'), 0o755)
copy('scripts/remote-deploy-20260924-v1.0.165-static.sh', path.join(stage, 'release-static.sh'), 0o755)
...
const miniStage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-165-mini-source.'))
