...
const BUILD_ID = '20260924-closure-1.0.167'
const PREVIOUS_BACKEND_VERSION = '1.0.165'
const PREVIOUS_BACKEND_JAR_SHA256 = '934d345dee88ea7b9d12f8aeff5998e97c1f8d447caff5945e41d25cafbbd7ef'
const PREVIOUS_STATIC_VERSION = '1.0.165'
const PREVIOUS_STATIC_COMMIT = '60a6e5e3fe1255bace2156f0f59089c9a341db0f'
...
if (process.argv.length !== 3 || process.argv[2] !== '--package-only') {
  throw new Error('Usage: node scripts/release-lingqi-167.mjs --package-only')
}
...
if (version !== VERSION || miniPackage.version !== VERSION || miniLock.version !== VERSION
  ...
  throw new Error('Root and mini-program versions must all equal 1.0.167')
}
...
const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-167-unified-candidate.'))
...
copy('scripts/remote-deploy-20260924-v1.0.167-backend.sh', path.join(stage, 'release-backend.sh'), 0o755)
copy('scripts/remote-deploy-20260924-v1.0.167-static.sh', path.join(stage, 'release-static.sh'), 0o755)
...
const miniStage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-167-mini-source.'))
