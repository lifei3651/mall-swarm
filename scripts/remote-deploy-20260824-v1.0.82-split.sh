...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.82
EXPECTED_VERSION=1.0.82
EXPECTED_PREVIOUS_VERSION=1.0.81
EXPECTED_GIT_COMMIT=73fab35b2cef0f35507a3b35aaa67951c5845e38
EXPECTED_BUILD_ID=20260824-1233-1.0.82
EXPECTED_JAR_SHA256=7a0a86479588c0f5c870f0a692b6ec61d4627b849ce18528cb95e811af30b55c
EXPECTED_PREVIOUS_MIGRATIONS=21
EXPECTED_MIGRATIONS=22
EXPECTED_NEW_MIGRATION=202608241530
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1082
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1082\`" >/dev/null 2>&1 || true' EXIT
    ...
    [[ "$(mysql_root -NBe "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='${verify_db}' AND table_name IN ('dms_member_message','dms_message_template','dms_message_channel_config','dms_message_delivery_task')")" == 0 ]]
  )
}
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.82 release failed; entering bounded recovery" >&2
...
[[ "$RELEASE_DIR" == /tmp/lingqimall-release-v1.0.82 ]]
for file in mall-distribution.jar admin.tar.gz shop.tar.gz team.tar.gz integrated.tar.gz VERSION RELEASE_MANIFEST.json SHA256SUMS production-backup.sh db-migrate.sh lingqimall.conf lingqimall-security.conf; do
  [[ -s "$RELEASE_DIR/$file" ]]
done
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.82.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.82.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.82.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.82.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  ...
  grep -q '"version": "1.0.82"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.82 startup log verification failed" >&2
  exit 1
fi
...
echo "release-success version=$EXPECTED_VERSION backup-before=$BACKUP_BEFORE backup-after=$BACKUP_AFTER build=$EXPECTED_BUILD_ID core-counts=$BEFORE_COUNTS migrations=$EXPECTED_MIGRATIONS message-seeds=15:15 external-channels=off encryption-preserved=yes jar=$EXPECTED_JAR_SHA256"
