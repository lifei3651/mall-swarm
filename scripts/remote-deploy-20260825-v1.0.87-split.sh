...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.87
EXPECTED_VERSION=1.0.87
EXPECTED_PREVIOUS_VERSION=1.0.84
EXPECTED_PREVIOUS_GIT_COMMIT=ef97831c175f89daff0de8eeb895ff575c1a3385
EXPECTED_PREVIOUS_BUILD_ID=20260824-1645-1.0.84
EXPECTED_GIT_COMMIT=7f66a83cdc10b44734c1d5788fbc8371758728d3
EXPECTED_BUILD_ID=20260825-0950-1.0.87
EXPECTED_JAR_SHA256=dcb9afed7063936f183c681e2f4ac3cc346f1e1ef9a1b8900add1ab43121a272
EXPECTED_PREVIOUS_MIGRATIONS=24
EXPECTED_MIGRATIONS=26
EXPECTED_NEW_ARRIVAL_MIGRATION=202608251130
EXPECTED_LIVE_RESERVATION_MIGRATION=202608251700
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1087
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1087\`" >/dev/null 2>&1 || true' EXIT
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.87 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.87"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success=1')" == "$EXPECTED_PREVIOUS_MIGRATIONS" ]]
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success<>1')" == 0 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version IN ('${EXPECTED_NEW_ARRIVAL_MIGRATION}','${EXPECTED_LIVE_RESERVATION_MIGRATION}')")" == 0 ]]
verify_notification_kernel
verify_real_name_schema
[[ "$(feature_schema_object_count)" == 0 ]]
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.87.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.87.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.87.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.87.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.87"' "$manifest"
...
[[ "$(mysql_cmd -NBe 'SELECT COUNT(*) FROM dms_schema_migration_history WHERE success<>1')" == 0 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version='${EXPECTED_NEW_ARRIVAL_MIGRATION}' AND success=1")" == 1 ]]
[[ "$(mysql_cmd -NBe "SELECT COUNT(*) FROM dms_schema_migration_history WHERE version='${EXPECTED_LIVE_RESERVATION_MIGRATION}' AND success=1")" == 1 ]]
verify_message_schema
verify_notification_kernel
verify_real_name_schema
verify_feature_schema
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.87 startup log verification failed" >&2
  exit 1
fi
