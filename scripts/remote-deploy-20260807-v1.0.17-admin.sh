...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.17-admin
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.17-admin
BACKUP_PATH=""
MUTATED=0
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.17" ]]
...
grep -R -Fq '手机版首页装修工作台' "$APP_ROOT/nginx/admin/assets"
...
echo "release-success version=1.0.17 backup=$BACKUP_PATH admin_entry=$admin_entry"
