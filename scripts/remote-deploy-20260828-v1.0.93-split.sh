...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.93
EXPECTED_VERSION=1.0.93
EXPECTED_PREVIOUS_VERSION=1.0.92
EXPECTED_PREVIOUS_GIT_COMMIT=9a774ccdc1a4cbf75fc72fd0481a8564a09098be
EXPECTED_PREVIOUS_BUILD_ID=20260827-1101-1.0.92
EXPECTED_GIT_COMMIT=132f2defa59e8e6051f24fec0d23752498ef6833
EXPECTED_BUILD_ID=20260828-0835-1.0.93
EXPECTED_JAR_SHA256=833d420b99b8ecf5a83de003934f5de9b78de9c52b467b4d25e4ac3652613577
...
verify_backup_restoreability() {
  local verify_db=mall_distribution_release_verify_1093
  local verify_create_sql="${DB_CREATE_SQL/\`$DB_NAME\`/\`$verify_db\`}"
  (
    trap 'mysql_root -e "DROP DATABASE IF EXISTS \`mall_distribution_release_verify_1093\`" >/dev/null 2>&1 || true' EXIT
    ...
    [[ "$(service_sms_preference_column_count_for "$verify_db")" == 0 ]]
  )
}
...
restore_database_from_backup() {
  ...
  verify_real_name_schema
  verify_feature_schema
  [[ "$(service_sms_preference_column_count)" == 0 ]]
...
rollback() {
  ...
  set +e
  if [[ "$code" != 0 ]]; then
    echo "1.0.93 release failed; entering bounded recovery" >&2
...
grep -Fq '"version": "1.0.93"' "$RELEASE_DIR/RELEASE_MANIFEST.json"
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.93.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.93.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.93.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.93.XXXXXX")"
...
for manifest_spec in "$NEW_ADMIN/version.json:admin" "$NEW_SHOP/version.json:storefront-public" "$NEW_TEAM/version.json:team-h5"; do
  manifest="${manifest_spec%%:*}"
  application="${manifest_spec##*:}"
  grep -q '"version": "1.0.93"' "$manifest"
...
if journalctl -u lingqimall-distribution.service --since '-3 minutes' --no-pager | grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' >/dev/null; then
  echo "1.0.93 startup log verification failed" >&2
  exit 1
fi
