...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.35
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.35
BACKUP_PATH=""
MUTATED=0
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.35" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.34" ]]
...
admin_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/admin/?release=1.0.35')
shop_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/?release=1.0.35')
...
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$admin_entry?release=1.0.35" >/dev/null
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$shop_entry?release=1.0.35" >/dev/null
grep -R -Fq '管理员核实客户身份后可直接代改' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '更换绑定手机号' "$APP_ROOT/nginx/shop/assets"
grep -R -Fq '请确认收款人信息无误' "$APP_ROOT/nginx/shop/assets"
...
echo "release-success version=1.0.35 backup=$BACKUP_PATH admin_entry=$admin_entry shop_entry=$shop_entry"
