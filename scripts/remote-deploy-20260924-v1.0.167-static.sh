#!/usr/bin/env bash
# Deploy exactly one immutable 1.0.167 static surface from the unified closure candidate.
set -Eeuo pipefail
...
EXPECTED_PREVIOUS_COMMIT=60a6e5e3fe1255bace2156f0f59089c9a341db0f
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-167\.[A-Za-z0-9]+$ ]] || fail "invalid candidate directory"
...
case "$SITE" in
  admin)
    ...
    PREVIOUS_BUILD_ID=20260924-closure-1.0.165
    ;;
  shop)
    LABEL=public-h5
...
assert manifest['previousStaticVersion'] == '1.0.165'
assert manifest['previousStaticCommit'] == '60a6e5e3fe1255bace2156f0f59089c9a341db0f'
...
assert candidate == {
    'version':'1.0.167', 'edition':'app-h5-split', 'application':application,
    'gitCommit':commit, 'buildId':build_id,
}
...
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "$EXPECTED_VERSION" ]] || fail "backend version is not 1.0.167"
...
node --check "$RETENTION_VERIFIER" >/dev/null \
...
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/$LABEL-167.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.$SITE-167-stage.XXXXXX")
...
query = '?release=1.0.167-' + commit[:12]
