...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-team-h5-137\.[A-Za-z0-9]+$ ]]
...
assert version == {'version':'1.0.137','edition':'app-h5-split','application':'team-h5','gitCommit':commit,'buildId':'20260909-team-h5-1.0.137'}
print('team-h5-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'team-h5-preflight=passed previous=1.0.136 target=1.0.137 backend=1.0.137'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/team-h5-137.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.team-137-stage.XXXXXX")
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "team-h5-release-success version=1.0.137 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
