...
if (version !== expectedVersion || pkg.version !== expectedVersion || lock.version !== expectedVersion
  ...
  throw new Error('根版本、小程序包版本和锁文件必须全部精确等于 1.0.158')
}
NODE
...
bash -n scripts/production-backup.sh scripts/db-migrate.sh \
  ...
  scripts/remote-deploy-20260923-v1.0.158-backend.sh \
  scripts/remote-deploy-20260923-v1.0.158-static.sh
node --check scripts/release-lingqi-158.mjs
node --check scripts/prepare-lingqi-mini-release.mjs
node --check scripts/upload-lingqi-mini-158.mjs
node --test scripts/tests/*.test.mjs
python3 - <<'PY'
from pathlib import Path

release = Path('scripts/remote-deploy-20260923-v1.0.158-backend.sh').read_text()
