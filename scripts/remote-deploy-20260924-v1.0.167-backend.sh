#!/usr/bin/env bash
# 1.0.167 closure backend release. No database migration may be applied.
set -Eeuo pipefail
...
EXPECTED_PREVIOUS_VERSION=1.0.165
EXPECTED_PREVIOUS_JAR_SHA=934d345dee88ea7b9d12f8aeff5998e97c1f8d447caff5945e41d25cafbbd7ef
EXPECTED_MIGRATIONS_BEFORE=41
...
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-closure-167\.[A-Za-z0-9]+$ ]] || fail "invalid release directory"
...
verify_unauthorized_contract() {
  local body status
  body=$(mktemp /tmp/lingqimall-closure-167-unauthorized.XXXXXX)
...
atomic_install() {
  ...
  temporary=$(mktemp "$(dirname "$destination")/.release-167.$(basename "$destination").XXXXXX") || return 1
...
recover() {
  ...
  if [[ "$status" != 0 && "$MUTATED" == 1 ]]; then
    if systemctl stop "$SERVICE" \
      ...
      echo "bounded-recovery=previous-backend-and-version-restored migration-ledger-unchanged=yes rollback=$ROLLBACK_DIR" >&2
    else
...
assert manifest['version'] == '1.0.167'
...
assert manifest.get('buildId') == build_id == '20260924-closure-1.0.167'
assert manifest.get('buildMethod') == build_method == 'clean-build-in-release-process'
assert manifest['previousVersion'] == '1.0.165'
assert manifest['previousJarSha256'] == '934d345dee88ea7b9d12f8aeff5998e97c1f8d447caff5945e41d25cafbbd7ef'
...
[[ "$MIGRATION_STATE" == 41:41 ]] || fail "production migration state mismatch: $MIGRATION_STATE"
...
probe_public
echo "release-preflight=passed previous=$EXPECTED_PREVIOUS_VERSION target=$EXPECTED_VERSION migrations=$MIGRATION_STATE migration-mode=verify-only"
...
ROLLBACK_DIR=$(mktemp -d /opt/lingqimall/backups/closure-backend-167.XXXXXX)
...
VERIFY_DB=m167v_$(date +%Y%m%d%H%M%S)
[[ "$VERIFY_DB" =~ ^m167v_[0-9]{14}$ ]] || fail "invalid verification database"
...
[[ "$(business_snapshot "$DB_NAME")" == "$STOPPED_DATABASE" ]] \
  || fail "production verification changed business state"
...
MUTATED=0
trap - EXIT
echo "release-success version=$EXPECTED_VERSION jar=$CANDIDATE_JAR_SHA backup-before=$BACKUP_BEFORE backup-after=$BACKUP_AFTER rollback=$ROLLBACK_DIR migrations=41:41 migration-mode=verify-only configuration-and-static-sites-preserved=yes"
