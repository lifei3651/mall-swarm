...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.74
EXPECTED_VERSION=1.0.74
EXPECTED_PREVIOUS_VERSION=1.0.62
EXPECTED_GIT_COMMIT=21ef651c82035a62ddfbfd94f55af1822c2435e1
EXPECTED_BUILD_ID=20260822-1901-1.0.74
EXPECTED_JAR_SHA256=76c5a7e20966ff691d8d25884eeea1310a950cd2b1a1a836034ccd2590866f37
EXPECTED_PREVIOUS_MIGRATIONS=13
...
rollback() {
  ...
  if [[ "$code" != 0 && "$APP_MUTATED" == 1 ]]; then
    echo "1.0.74 release failed; restoring 1.0.62 application files" >&2
...
[[ "$RELEASE_DIR" == /tmp/lingqimall-release-v1.0.74 ]]
for file in mall-distribution.jar admin.tar.gz shop.tar.gz team.tar.gz VERSION SHA256SUMS production-backup.sh db-migrate.sh; do
  [[ -s "$RELEASE_DIR/$file" ]]
done
[[ -x "$RELEASE_DIR/production-backup.sh" ]]
[[ -x "$RELEASE_DIR/db-migrate.sh" ]]
...
ROLLBACK_DIR="$(mktemp -d /tmp/lingqimall-rollback-v1.0.74.XXXXXX)"
...
NEW_ADMIN="$(mktemp -d "$APP_ROOT/nginx/.admin-v1.0.74.XXXXXX")"
NEW_SHOP="$(mktemp -d "$APP_ROOT/nginx/.shop-v1.0.74.XXXXXX")"
NEW_TEAM="$(mktemp -d "$APP_ROOT/nginx/.team-v1.0.74.XXXXXX")"
...
for manifest_spec in \
  ...
  grep -q '"version": "1.0.74"' "$manifest"
...
[[ "$(curl --http1.1 -sS -o /dev/null -w '%{http_code}' --max-time 12 -X POST -H 'Content-Type: application/json' -d '{}' 'https://lingqimall.com/api/distribution/admin-auth/step-up')" == 401 ]]
...
if grep -E 'Application run failed|OutOfMemoryError|Access denied for user|UnsatisfiedDependencyException|BeanCreationException| ERROR ' "$NEW_LOG_OUTPUT" >/dev/null; then
  echo "1.0.74 startup log verification failed" >&2
  exit 1
fi
