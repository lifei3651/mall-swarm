...
const BUILD_ID = '20260921-closure-1.0.154'
const PREVIOUS_BACKEND_VERSION = '1.0.153'
const PREVIOUS_BACKEND_JAR_SHA256 = 'bb2ef6a1c3c8fe908f088b16ac86fbce193334645e83c343adbd4392d2b93009'
const PREVIOUS_STATIC_VERSION = '1.0.153'
const PREVIOUS_STATIC_COMMIT = '8d94be83088f3ed8aab8664a7a13eccdffc4cb41'

if (process.argv.length !== 3 || process.argv[2] !== '--package-only') {
  throw new Error('Usage: node scripts/release-lingqi-154.mjs --package-only')
}
...
if (version !== VERSION || miniPackage.version !== VERSION || miniLock.version !== VERSION
  ...
  throw new Error('Root and mini-program versions must all equal 1.0.154')
}
...
const migrations = fs.readdirSync('document/db/migrations')
  ...
  .sort()
if (migrations.length !== 40 || migrations[0] !== 'V202608111205__tenant_config_versions_and_operation_log_index.sql'
  || migrations.at(-1) !== 'V202609202100__system_fund_accounts.sql') {
  throw new Error('Unexpected migration inventory')
}

const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-154-unified-candidate.'))
...
copy('scripts/remote-deploy-20260921-v1.0.154-backend.sh', path.join(stage, 'release-backend.sh'), 0o755)
copy('scripts/remote-deploy-20260921-v1.0.154-static.sh', path.join(stage, 'release-static.sh'), 0o755)
...
const miniStage = fs.mkdtempSync(path.join(os.tmpdir(), 'lingqi-154-mini-source.'))
