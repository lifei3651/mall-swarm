...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-public-h5-138\.[A-Za-z0-9]+$ ]]
[[ "$EXPECTED_COMMIT" =~ ^[a-f0-9]{40}$ ]]
[[ "$MODE" == --preflight-only || "$MODE" == --authorize-release ]]
[[ "${LINGQIMALL_RELEASE_AUTHORIZATION:-}" == 1.0.138 ]]
...
assert old['version'] == '1.0.137' and old['gitCommit'] == 'ef47181abe9a1ad67f01f0ced92f0540caca57ab'
...
assert version == {'version':'1.0.138','edition':'app-h5-split','application':'storefront-public','gitCommit':commit,'buildId':'20260911-public-h5-1.0.138'}
print('public-h5-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'public-h5-preflight=passed previous=1.0.137 target=1.0.138 backend=1.0.137'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/public-h5-138.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.shop-138-stage.XXXXXX")
...
with urllib.request.urlopen(base + '/version.json?release=1.0.138', timeout=12) as response: version = json.load(response)
assert version['version'] == '1.0.138' and version['gitCommit'] == sys.argv[1]
with urllib.request.urlopen(base + '/?release=1.0.138', timeout=12) as response: html = response.read().decode()
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "public-h5-release-success version=1.0.138 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
