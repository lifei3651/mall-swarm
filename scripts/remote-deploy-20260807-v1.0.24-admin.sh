...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.24-admin
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.24-admin
BACKUP_PATH=""
MUTATED=0
...
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.24" ]]
...
sleep 2
admin_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' -H 'Pragma: no-cache' 'https://lingqimall.com/admin/?release=1.0.24')
grep -q '<div id="app">' <<< "$admin_html"
admin_entry=$(grep -o '/admin/assets/index-[^" ]*\.js' <<< "$admin_html" | head -1)
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$admin_entry?release=1.0.24" >/dev/null
grep -R -Fq '首页轮播图管理' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '首页模块' "$APP_ROOT/nginx/admin/assets"
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.24" ]]
systemctl is-active --quiet nginx
systemctl is-active --quiet mysql
systemctl is-active --quiet redis-server
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' -H 'Pragma: no-cache' 'https://lingqimall.com/?release=1.0.24' >/dev/null

MUTATED=0
trap - EXIT
rm -rf -- "$RELEASE_DIR" "$ROLLBACK_DIR"
echo "release-success version=1.0.24 backup=$BACKUP_PATH admin_entry=$admin_entry"
