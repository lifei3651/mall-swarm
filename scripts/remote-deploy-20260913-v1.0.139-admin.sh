...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-admin-139\.[A-Za-z0-9]+$ ]]
...
assert version == {'version':'1.0.139','edition':'app-h5-split','application':'admin','gitCommit':commit,'buildId':'20260913-admin-1.0.139'}
print('admin-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'admin-preflight=passed previous=1.0.137 target=1.0.139 backend=1.0.139'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/admin-139.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.admin-139-stage.XXXXXX")
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "admin-release-success version=1.0.139 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
