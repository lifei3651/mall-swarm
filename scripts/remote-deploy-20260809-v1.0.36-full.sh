...
RELEASE_DIR=/tmp/lingqimall-release-v1.0.36
ROLLBACK_DIR=/tmp/lingqimall-rollback-v1.0.36
...
rollback_schema() {
  [[ "$SCHEMA_RENAMED" == 1 ]] || return 0
  local has_username has_login_account has_old_index has_new_index
  has_username=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND COLUMN_NAME='username'" "$DB_NAME")
  has_login_account=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND COLUMN_NAME='login_account'" "$DB_NAME")
  if [[ "$has_username" == 0 && "$has_login_account" == 1 ]]; then
    MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" "$DB_NAME" -e "ALTER TABLE dms_shop_member CHANGE COLUMN login_account username varchar(64) DEFAULT NULL COMMENT '用户名'"
  fi
  has_old_index=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND INDEX_NAME='uk_username'" "$DB_NAME")
  has_new_index=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND INDEX_NAME='uk_login_account'" "$DB_NAME")
  if [[ "$has_old_index" == 0 && "$has_new_index" -gt 0 ]]; then
    MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" "$DB_NAME" -e "ALTER TABLE dms_shop_member RENAME INDEX uk_login_account TO uk_username"
  fi
}
...
rollback() {
  ...
  if [[ "$code" != 0 && "$MUTATED" == 1 ]]; then
    echo "release failed; restoring previous application and member schema" >&2
...
[[ -s "$RELEASE_DIR/20260808_rename_member_login_account.sql" ]]
[[ -s "$RELEASE_DIR/SHA256SUMS" ]]
[[ "$(tr -d '[:space:]' < "$RELEASE_DIR/VERSION")" == "1.0.36" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.35" ]]
...
has_username=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND COLUMN_NAME='username'" "$DB_NAME")
has_login_account=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND COLUMN_NAME='login_account'" "$DB_NAME")
[[ "$has_username" == 1 && "$has_login_account" == 0 ]]
...
MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" "$DB_NAME" < "$RELEASE_DIR/20260808_rename_member_login_account.sql"
SCHEMA_RENAMED=1

has_username=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND COLUMN_NAME='username'" "$DB_NAME")
has_login_account=$(MYSQL_PWD="$DB_PASSWORD" mysql -h127.0.0.1 -u"$DB_USER" -NBe "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$DB_NAME' AND TABLE_NAME='dms_shop_member' AND COLUMN_NAME='login_account'" "$DB_NAME")
[[ "$has_username" == 0 && "$has_login_account" == 1 ]]
...
admin_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/admin/?release=1.0.36')
shop_html=$(curl -fsS --max-time 12 -H 'Cache-Control: no-cache' 'https://lingqimall.com/?release=1.0.36')
...
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$admin_entry?release=1.0.36" >/dev/null
curl -fsS --max-time 12 -H 'Cache-Control: no-cache' "https://lingqimall.com$shop_entry?release=1.0.36" >/dev/null
...
grep -R -Fq '确认删除分类' "$APP_ROOT/nginx/admin/assets"
grep -R -Fq '账号注册成功' "$APP_ROOT/nginx/shop/assets"
grep -R -Fq '短信验证码错误，请重新输入' "$APP_ROOT/nginx/shop/assets"
[[ "$(database_counts)" == "$BEFORE_COUNTS" ]]
[[ "$(tr -d '[:space:]' < "$APP_ROOT/VERSION")" == "1.0.36" ]]
systemctl is-active --quiet lingqimall-distribution.service
systemctl is-active --quiet nginx
systemctl is-active --quiet mysql
systemctl is-active --quiet redis-server
...
echo "release-success version=1.0.36 backup=$BACKUP_PATH admin_entry=$admin_entry shop_entry=$shop_entry core-counts=$BEFORE_COUNTS"
