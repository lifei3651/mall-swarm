...
MODE=${2:-}
EXPECTED_COMMIT=${3:-}
[[ "$RELEASE_DIR" =~ ^/tmp/lingqimall-admin-136\.[A-Za-z0-9]+$ ]]
...
assert old['version'] == '1.0.132' and old['gitCommit'] == '91e5a6d824be2f6e7a3f6f9f5d642b7f9635c957'
...
assert version == {'version':'1.0.136','edition':'app-h5-split','application':'admin','gitCommit':commit,'buildId':'20260909-admin-1.0.136'}
print('admin-candidate-and-previous-version=passed')
PY
BEFORE_FILES=$(protected_hashes)
echo 'admin-preflight=passed previous=1.0.132 target=1.0.136 backend=1.0.136'
[[ "$MODE" != --preflight-only ]] || exit 0
BACKUP_BEFORE=$(backup_verify)
ROLLBACK=$(mktemp -d "$APP_ROOT/backups/admin-136.XXXXXX")
STAGE=$(mktemp -d "$APP_ROOT/nginx/.admin-136-stage.XXXXXX")
...
for svc in lingqimall-distribution nginx mysqld redis; do systemctl is-active --quiet "$svc"; done
MUTATED=0
echo "admin-release-success version=1.0.136 source=$EXPECTED_COMMIT before=$BACKUP_BEFORE after=$BACKUP_AFTER rollback=$ROLLBACK backend-other-sites-config-preserved=yes"
