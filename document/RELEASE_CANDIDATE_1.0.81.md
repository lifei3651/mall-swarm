...
状态：正式候选已生成并完成本地校验，**未发布**

生产现状：公开商城、团队 H5、管理后台只读核验均为 `1.0.80`
...
- [ ] 公开商城、团队 H5、管理后台 `version.json` 均为 `1.0.81`、冻结 Git 和统一构建号；一体化 H5 未被意外启用。
- [ ] 三个入口及其入口资源返回 200，公开商城和团队 H5 首页 API 正常。
- [ ] 后端健康 `UP`，Nginx、MySQL、Redis、分销服务均 active，Redis `PONG`，启动后无新增严重错误。
- [ ] `/api/shop/home`、直播广场、新品速递返回正常；公开商品接口不暴露成本、BV、安全库存、结算及商户内部字段。
- [ ] 使用非法手机号对 `/api/sms/send/login` 做无费用探测，返回正确手机号校验提示且不发送真实短信。
- [ ] 浏览器人工确认：公开商城“验证码登录”不显示图形验证码并走登录短信；注册显示图形验证码并走注册短信；密码登录显示图形验证码。团队 H5 与一体化流程不做拓扑或行为变更。
- [ ] 未登录后台接口为 401；Actuator、Swagger、`.env`、`.git` 等敏感路径对公网为 404。
- [ ] CSP、Permissions-Policy、HSTS 等安全响应头存在。
- [ ] 迁移保持 21/21、失败 0；核心业务计数发布前后一致；待处理奖金、ERP、移线和超时订单为 0。
- [ ] 明文敏感字段计数为 0；加密环境文件和 systemd drop-in 的 SHA-256 发布前后一致。
- [ ] 发布前、发布后两份完整备份均通过哈希、压缩和关键对象清单复验。

## 发布判定

当前没有已知发布阻断。候选包、版本专用发布脚本、备份对象、顺序、回滚点和验收门禁均已准备完成，可以进入“用户明确授权后按脚本受控发布”的状态。若授权时线上版本、迁移、服务、数据计数、加密配置或远程仓库身份发生变化，发布脚本必须自动停止并重新评估，不能绕过门禁。
