...
本回执仅证明 P0-10 对 1.0.158 通过。**留档完成当时**正式服务器仍为 1.0.154、迁移 40/40，尚未运行 1.0.158 的备份、隔离迁移、正式部署、微信上传或真机交易。之后取得单独 S 授权并完成的服务器发布另见 `SERVER_RELEASE_20260923.md`；本回执 JSON 中的 `gates` 记录留档阶段的授权边界，不代表后续服务器当前状态。微信 W 与真实 R 仍须独立授权、执行和复核。
