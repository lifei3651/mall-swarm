...
| 支付宝异步通知 | `POST /api/v1/pay/alipay/notify` | 支付宝签名、应用/商户/金额核验 | 仅 `TRADE_SUCCESS` 入账；订单状态机防重复；超时关单后的迟到支付用稳定退款号退款并持久化 `late_refund_flag` | 已入账或已完成迟到退款的通知均返回 `success`；渠道退款或本地标记失败才返回 `failure` |
