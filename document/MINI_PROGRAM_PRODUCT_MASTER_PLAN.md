...
更新：2026-09-08；负责人：当前 Codex 总控。最初基线：`776b4f2f` / 1.0.128；最新发布状态见下方及RELEASE_REPORT。适用：通用商城基座的原生小程序，不是某个客户的正式交付验收。

9月8日收尾：已补账号/短信替代、注册/找回、原生更新管理、受角色限制的主播台、订单折叠/分包裹及多页生命周期保护；全部27页、四首页/三导购补隔离原生截图与小屏复查。当前状态以[64项附件的9月8日增量](MINI_H5_FULL_PARITY_133.md#2026-09-08-未完成项收尾本地完成状态优先于下方历史表)为准，下方早期“未实现”保留历史。396项原生、111项相关后端、104项H5自动回归通过不等于真实渠道验收；新版后端/小程序未发布，正式目录未动，真机授权及资金门禁仍须实测。

## 1. 总控结论与职责

2026-09-12用户明确反馈规范：**要求显眼弹窗的是错误，不是每一次正常成功**。地址导入/保存直接更新列表或返回结算；登录/加购不补成功弹窗；message和登录引导留在页面。其余普通成功最多使用短暂非阻断轻提示，错误、结果未知、风险说明及删除/清空/付款等必要确认保留。此规则覆盖下文历史“操作消息均需确认”的理解，不能再次将成功提示统一包装为“知道了”。

2026-09-08用户补充执行规则：**同类问题双端检查、适用端同步修复、分别验收**。H5是已验证功能基准，不代表H5没有Bug或禁止修改；确认H5也缺失必要交互或存在缺陷时，必须一起修，记录影响端和发布状态。优先复用共享规则，微信授权、支付、地址及生命周期继续单独适配。不得因为“先前只改小程序”遗漏H5，也不得因一端测试通过宣称另一端通过。本轮搜索历史落实情况见64项附件最新记录。

2026-09-07用户最新要求：**已修好的公开H5是全部前端功能、UI层级和交互逻辑的基准**。此前“按微信环境重新组织操作”不能成为减少功能、另造加购/购物车流程或漏页面的理由。仅微信授权、原生支付、地址导入等平台能力保留必要适配；用户已明确的显眼弹窗、本人会员和奖金入口保留。团队H5非公开权限不因此暴露。当前完整执行附件为[64项逐页对照清单](MINI_H5_FULL_PARITY_133.md)，其初始盘点、已改、自动/原生/真机验收分别登记。

此前按用户逐个指出的问题做修补、把样式统一放在功能清单末尾，缺少先确定页面职责和用户路径的过程；189项或300项自动测试通过也不能证明全部体验和交易能上线。下方早期版本记录保留历史，不代表本轮全量对齐已经完成。
...
2026-09-07最新增量：132已部署/上传的事实以RELEASE_REPORT为准，下表旧版本条目保留开发时证据。132后的本地修复已覆盖购物主链、评价/工单/余额/账号分层/直播消费者互动，并补授权返回和邀请闪动保护；352项小程序回归及官方30/32编译通过，仅部分320/390原生UI验收。**并非全量完成，也未上传下一版**。每一项最新状态及未完成项以[64项执行表](MINI_H5_FULL_PARITY_133.md#2026-09-07-本轮执行结果未发布)为准；短信账号替代/找回、主播台、完整模板及真机渠道仍未完成。
...
| MC07 | P0 / 订单支付恢复 | 详情补正确待付/实付说明与继续付款；联合单/子单路径核验 | 从各入口找回同一订单，不重复下单；未知结果不显示已付 | 132详情续付/完整合并总额/状态标签已补，列表统一去详情核对；300项自动及部分320/390原生隔离实点通过。真实支付恢复、迟到回调仍依赖MC05/08，不判整项完成；原05、06 |
...
| MC12 | P1 / 装修与内容 | 建后台字段→原生模板映射矩阵，限制不可用跳转 | 隔离配置逐项可测，不用改生产或关域名校验掩盖 | 活动信息流版公开活动/后台预览同步已补并完成部分隔离实测；其余组合、390末轮和真机待验；原18、19 |
...
| MC14 | P1 / H5公开功能对齐 | 用户已明确纳入商品评价与直播消费者互动，主播台按受控角色单列 | 同H5订单评价资格、评论/预约规则；内容安全、媒体和分享真机独立验收 | 本地已补评价/预约/评论/统计并自动通过；原生完整视觉及真机未验，主播台尚未实现，不能整项判完成；原20、23 |
| MC15 | P0 / 发版闸门 | 候选冻结、后台兼容、权限/隐私/订单配置、体验版与真机清单 | 启用的注册→下单→支付→发货→售后→退款闭环可证；无P0未决 | 9月7日经用户授权，后端/后台132已部署、真实源码132已上传并覆盖体验版；未提审/正式发布，未开启未验收的资金通道，真机与上述依赖仍待。不是整项完成；原12、26、27 |
...
### 最新发布增量：1.0.132（2026-09-07）

- 用户明确授权后，后端/后台09:34–09:38部署、原生09:44上传开发/体验候选完成。固定正式目录为主项目`dist/wechat-mini-program`，不再从QA临时副本上传；业务冻结91e5a6d8，自动回归698/300/102/175及编译通过，完整备份/隔离恢复与线上健康验证完成，详见RELEASE_CANDIDATE_1.0.132.md。
- 真实首页、游客我的、协议默认未选登录弹窗已查看，未授权、下单或实际转款。未提审/正式发布；MC01–14未完成项不会因上传而自动变为通过。此前条目“未发布”是当时开发状态，当前分层状态以此条及发布报告为准。

### 开发增量：1.0.132（2026-09-07跨端复核与订单恢复，修复当时未发布）

- 延续22页功能对照附件，区分业务必须一致、微信原生适配、尚缺功能，补当前逐项执行清单。不重新建第二套排期，不把源码清单当全流程真机通过。
- 列表和详情待付款金额改按状态显示，列表统一进入详情核对；详情补续付、完整合并订单总额、取消整笔交易提示、配置和最新订单检查、换号/晚预支付响应及防重复。未知结果不假报成功，不重复创建业务订单。
- 小程序300、H5 102、工程检查及官方26/27通过。原生390列表→详情→显眼失败弹窗与320详情已验，均为假数据隔离，无真实资金/生产操作；MC05/08及大字/真机等门禁不解除。见[本次证据](qa/2026-09-06-order-payment-alignment/README.md)。
- 本次只改小程序，不部署或上传。此前排序/本人奖金等132配套后端仍待发布，旧条目描述的是当时状态，以本最新增量为准。

### 1.0.132（商品详情服务保障排版，未发布）

- 修复服务标题与整段说明全部被样式套成胶囊的问题；改为逐项图文结构、深色标题、灰色完整说明及细分隔。保留已有服务配置与售后规则，只改原生商品详情样式/结构，不改交易逻辑和H5。
- 小程序285/H5 102、工程检查及官方26/27编译通过；Mac锁屏已请求解锁，隔离样例已准备，但320/390最终视觉与大字号未验。见[服务保障验收记录](qa/2026-09-06-product-service-layout.md)，不解除132发布门禁。

### 1.0.132（活动信息流版前后台同步，未发布）

- 后台“整体版型 / 首页模块”明确展示限时活动来源、管理入口、刷新及匹配数量；公开活动价/开始结束倒计时只出现在对应商品卡，去除此前虚构示意和原生空标题。使用现有公开活动与商品接口，不增加重复开关，不改H5和交易/限购后端。
- 小程序281、后台175、H5 102、后台构建、工程检查、官方26/27编译通过；后台四状态和原生320活动价/对应活动入口已实点。390切换期间锁屏，末次计时/循环标识可视复核及真机门禁保留；不是全量MC12通过。见[活动同步验收](qa/2026-09-06-campaign-preview-sync/README.md)。
- 本地132候选，不部署/上传或操作线上活动配置；此前排序/本人奖金后端依赖不变。

### 1.0.132（跨端功能核对、加购与累计限购，未发布）
