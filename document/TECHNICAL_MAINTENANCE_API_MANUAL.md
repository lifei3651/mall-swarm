...
- 公开普通注册写入 `team_opt_in=0`；从有效邀请链接或二维码进入时，公开注册在同一事务中绑定邀请人并写入 `team_opt_in=1`。团队 H5 不允许普通账号事后补绑关系。
...
| `POST /api/v1/shop/public/auth/register` | `phone`、`username`、`password`、`smsCode`、邀请链接可带 `inviteCode` | 登录/会员会话结果 | 普通入口创建 `team_opt_in=0` 的购物账号；有效邀请入口在注册交易内绑定邀请人并写入 `team_opt_in=1`，随后进入公开商城 |
...
| `POST /api/v1/shop/auth/login` | `account`、`loginType=password|sms`；密码方式传 `password`、`captchaId`、`captchaCode`，短信方式传 `smsCode` | 登录/会员会话结果 | 校验账号状态和锁定次数；团队入口额外要求 `team_opt_in=1`，成功后使旧会话失效 |
| `GET /api/v1/shop/auth/me` | 会话 Cookie、`X-Shop-Surface` | 当前会员资料和权限 | 校验会话和入口资格，可完成旧 Token 向 Cookie 的一次性迁移 |
...
| `GET /api/v1/shop/public/inviter-preview/{inviteCode}` | 路径邀请码 | 邀请人预览 | 注册前展示脱敏邀请人，不返回敏感资料 |
...
| `POST /api/v1/shop/wallet/withdrawals` | `withdrawAmount`、`withdrawType` 2微信/3支付宝、支付宝 `bankAccount`、`accountName`、`paymentPassword`、`smsCode`；请求键 | `WithdrawRecordVO` | 锁定会员资金范围，校验自然日/月申请次数、单笔人工审核阈值、首次/换号风险、余额、支付密码和短信；低风险申请自动通过并发起官方渠道打款，风险单等待一次人工审核；重复业务请求号不重复扣款 |
...
| 提现风控 | `SHOP_WITHDRAWAL_DAILY_MAX_COUNT`、`SHOP_WITHDRAWAL_MONTHLY_MAX_COUNT`、`SHOP_WITHDRAWAL_MANUAL_REVIEW_THRESHOLD` | 默认 2 次/日、10 次/月，单笔超过 1000 元进入一次人工审核；首次提现和支付宝换收款账号也人工审核。审核通过后系统直接发起官方渠道打款，不设置日/月累计金额上限 |
...
| 外部服务通知 | `EXTERNAL_NOTIFICATION_*`、`NOTIFICATION_SMS_*`、数据库三层预算和用户授权摘要 | 与验证码短信完全隔离；默认关闭；预算、模板、授权、终端任一缺失即安全抑制 |
...
- 当前功能基座重点迁移：租户配置版本与操作日志索引、售后期限配置、秒杀与复购基座、订单客服备注、直播广场、商品首次上架时间、统一站内消息及外部通知发送内核。
- 外部通知迁移新增任务租约字段、逐次尝试、三层预算、用户授权/终端摘要和回执摘要。迁移不写任何客户密钥、手机号、终端令牌或真实授权，预算默认零且关闭。
