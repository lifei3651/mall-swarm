...
case "${1:-}" in
  ...
  -h|--help) echo "用法: $0 [--without-team-h5]"; exit 0 ;;
  *) echo "用法: $0 [--without-team-h5]" >&2; exit 2 ;;
esac
[ "$#" -eq 0 ] || { echo "构建参数多余" >&2; exit 2; }
...
restore_candidates
(cd "$ROOT_DIR/mall-shop-web" && npm ci && npm test -- --run)
if [ "$TEAM_H5_ENABLED" = "true" ]; then
  (cd "$ROOT_DIR/mall-shop-web" && npm run build)
else
  (cd "$ROOT_DIR/mall-shop-web" && npm run build:public)
fi
(cd "$ROOT_DIR/mall-distribution-admin" && npm ci && npm test -- --run && npm run build)

mkdir -p "$STAGING/html/public" "$STAGING/html/admin"
cp -R "$ROOT_DIR/mall-shop-web/dist/." "$STAGING/html/public/"
if [ "$TEAM_H5_ENABLED" = "true" ]; then
  mkdir -p "$STAGING/html/team" "$STAGING/html/integrated"
...
if [ "$TEAM_H5_ENABLED" = "true" ]; then
  ...
  echo "仅公开商城、后台和后端生产构建完成（未包含团队H5/一体化H5），版本：${version}，批次：${release_build_id}"
fi
