...
public class MockAppPushAdapter implements ExternalNotificationAdapter {
    private final ExternalNotificationProperties external;
    private final MockNotificationProperties mock;
    @Override public String channel() { return "APP_PUSH"; }
    ...
    @Override public GateDecision readiness(ExternalNotificationContext context) {
        return external.isEnabled() && external.isWorkerEnabled() && mock.isEnabled() && mock.isAppPushEnabled()
    ...
    @Override public DeliveryResult send(ExternalNotificationContext context, String idempotencyKey) { return DeliveryResult.accepted("mock-app-" + context.getTaskId(), BigDecimal.ZERO); }
