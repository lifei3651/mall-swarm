...
public interface DmsErpSyncTaskDao {
    ...
    List<DmsErpSyncTask> selectRetryable(@Param("now") LocalDateTime now, @Param("limit") Integer limit);
    int insert(DmsErpSyncTask entity);
    int markSuccess(@Param("id") Long id, @Param("response") String response);
    int markFailure(@Param("id") Long id, @Param("retryCount") Integer retryCount, @Param("nextRetryTime") LocalDateTime nextRetryTime, @Param("error") String error);
}
