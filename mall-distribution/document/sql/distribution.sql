...
CREATE TABLE `dms_agent` (
  ...
  `id_card` varchar(256) DEFAULT NULL COMMENT '身份证号（应用层加密存储）',
  `bank_name` varchar(64) DEFAULT NULL COMMENT '开户行',
  `bank_account` varchar(512) DEFAULT NULL COMMENT '银行账号（应用层加密存储）',
...
CREATE TABLE `dms_tenant` (
  ...
  `promotion_join_mode` varchar(32) NOT NULL DEFAULT 'DISABLED' COMMENT '推广资格开通方式',
...
CREATE TABLE `dms_bonus_calculation_task` (
  ...
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_bonus_task_order` (`order_id`),
...
-- 21A. 微信小程序会员身份绑定表
-- ============================================================
DROP TABLE IF EXISTS `dms_wechat_mini_program_identity`;
CREATE TABLE `dms_wechat_mini_program_identity` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `tenant_id` bigint NOT NULL DEFAULT 1 COMMENT '租户ID',
...
CREATE TABLE `dms_shop_order` (
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '客户下单备注',
  `service_remark` varchar(500) DEFAULT NULL COMMENT '客服内部备注，不向客户展示',
