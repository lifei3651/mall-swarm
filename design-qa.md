...
## 分类导购 A/B/C 三版型验收（2026-08-25）

- Source visual truth: `/tmp/lingqi-category-guide/revised-a-blue.png`、`revised-b-blue.png`、`revised-c-blue.png`，分别对应双栏目录、视觉橱窗和场景导购。
- Implementation screenshots: `document/qa/2026-08-25-category-guide-a-implementation.png`、`2026-08-25-category-guide-b-implementation.png`、`2026-08-25-category-guide-c-implementation.png`。
- Viewport/state: Codex 应用内浏览器 415 × 869 CSS px；本地只读代理复用线上公开分类与在售商品数据，仅替换本次待发布展示配置。三种子版型的全部专属模块开启，购物车与账号入口显示。

### Full-view comparison evidence

- A 与参考图保持左侧一级目录、右侧分类主视觉、子分类和双列热销商品的层次；B 使用两列大图品类、横向货架和推荐商品；C 使用三张纵向购物场景、分类快捷入口和人气商品，三者结构差异真实存在于公开商城分类路由。
- 三版型共享深品牌蓝 `#1556A3`、珊瑚红 `#E5484D`、浅灰背景、白卡、14px 圆角、8px 间距节奏和现有中文字体栈，没有新增品牌标志。参考图中的示意商品被现有客户真实分类和商品图片替换。
- 参考图为 852/853 × 1844/1846 的长页效果，实现证据按当前浏览器可见视口截取；对照重点为首屏信息架构、色彩、卡片密度和交互层级，不把真实业务内容与参考图虚构素材的差异记为缺陷。

### Findings and comparison history

- [P2，已修复] 初版受旧租户主题影响，搜索按钮和底部选中态仍为红/绿；分类导购主版型现在在页面外壳统一覆盖为深品牌蓝。
- [P2，已修复] 初版场景标题优先使用后台分类备注，出现“首页推荐分类”等运营术语；现固定为“日常补充 / 精致生活 / 送礼优选”，并把真实分类名放入辅助说明。
- [P1，已修复] 旧 `bottomNav` 若完全缺少购物车或账号项，仅强制已有项开启仍可能失去必备入口；前端迁移会补回缺项，后端保存同时拒绝关闭或移除。
- [P2，已修复] A 首次截图在商品图片解码完成前采样，出现白色图片区；最终截图等待真实商品图片 `naturalWidth` 可用后重拍。
- 最终未发现仍需处理的 P0、P1 或 P2 视觉问题；A、B、C 均通过。

### Primary interactions tested

1. A 一级分类、B 大图品类/货架、C 场景卡和快捷分类均可切换真实筛选结果。
2. 三版型商品卡进入真实商品详情，并使用现有库存、登录和购买限制规则加入购物车。
3. 全部分类导购模块关闭时显示说明并保留安全商品列表，不出现空白页；父版型切换不修改或清空子模块原值。
4. 三版型均加载 5 个真实商品卡，无兜底警告；购物车与“我的”入口存在，控制台错误为 0。
